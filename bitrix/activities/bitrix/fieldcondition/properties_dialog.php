		...
		<tr id="bwffc_deleterow_tr_<?= $i ?>">
			<td align="right" width="40%" class="adm-detail-content-cell-l">
				<select name="field_condition_joiner_<?=$i?>">
					...
					<option value="1" <?php if($arCurrentValues["field_condition_joiner_".$i]==1) echo 'selected'?>><?= GetMessage("BPFC_PD_OR") ?></option>
				</select>
			</td>
		...
		<td width="60%" class="adm-detail-content-cell-r">
			<select name="field_condition_field_<?= $i ?>" data-old="<?=htmlspecialcharsbx($arCurrentValues["field_condition_field_".$i])?>" onchange="BWFFCSetAndChangeFieldType(<?= $i ?>, this)">
				...
				foreach ($arDocumentFields as $key => $value)
				{
					...
					?><option value="<?= htmlspecialcharsbx($key) ?>"<?= ($arCurrentValues["field_condition_field_".$i] == $key) ? " selected" : "" ?>><?= htmlspecialcharsbx($value["Name"]) ?></option><?php 
				}
				?>
			</select>
		</td>
	...
	<tr>
		...
		<td width="60%" class="adm-detail-content-cell-r">
			<select name="field_condition_condition_<?= $i ?>" onchange="BWFFCChangeCondition(<?= $i ?>, this.options[this.selectedIndex].value)">
				...
				{
					?><option value="<?= $key ?>"<?= ($arCurrentValues["field_condition_condition_".$i] == $key) ? " selected" : "" ?>><?= $value ?></option><?php 
				}
				?>
			</select>
		</td>
	</tr>
	<?php  $hidden = in_array($arCurrentValues["field_condition_condition_".$i], ['modified', 'empty', '!empty']);?>
	<tr id="id_tr_field_condition_value_<?= $i ?>" style="<?php if ($hidden) echo 'display:none'?>">
