...
<tr>
	...
	<td width="60%">
		<select name="entity_type" onchange="BPDUA_changeEntityType(this.value);">
			<option value="user" <?php if ($arCurrentValues['entity_type'] == 'user') echo 'selected'?>><?=GetMessage('BPDUA_PD_ENTITY_TYPE_USER')?></option>
			<?php  if (CModule::IncludeModule('socialnetwork')):?>
			<option value="sg" <?php if ($arCurrentValues['entity_type'] == 'sg') echo 'selected'?>><?=GetMessage('BPDUA_PD_ENTITY_TYPE_SG')?></option>
			<?php endif?>
			<option value="common" <?php if ($arCurrentValues['entity_type'] == 'common') echo 'selected'?>><?=GetMessage('BPDUA_PD_ENTITY_TYPE_COMMON')?></option>
			<option value="folder" <?php if ($arCurrentValues['entity_type'] == 'folder') echo 'selected'?>><?=GetMessage('BPDUA_PD_ENTITY_TYPE_FOLDER_1')?></option>
		</select>
	</td>
</tr>
