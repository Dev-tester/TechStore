...
<?php 
	if ($GLOBALS["USER"]->IsAuthorized())
	{
		?><span class="forum-option-additional"><a href="#postform" onclick="ShowFirstPost(this); return false;"><?=(
			$arUserSettings["first_post"] == "show" ? GetMessage("F_COLLAPSE") : GetMessage("F_SHOW"))?></a></span><?php 
	}
	...
	if ($USER->IsAuthorized())
	{
		if (empty($arResult["USER"]["SUBSCRIBE"]))
		{
			...
			<noindex><a rel="nofollow" title="<?=GetMessage("F_SUBSCRIBE_TITLE")?>" href="<?php 
			?><?=$APPLICATION->GetCurPageParam("TOPIC_SUBSCRIBE=Y&".bitrix_sessid_get(), array("FORUM_SUBSCRIBE", "FORUM_SUBSCRIBE_TOPIC", "sessid"))?><?php 
				?>"><?=GetMessage("F_SUBSCRIBE")?></a></noindex></span><?php 
		}
		else
		{
			?>&nbsp;&nbsp;<span class="forum-option-subscribe forum-option-do-unsubscribe"><noindex><a rel="nofollow" title="<?=GetMessage("F_UNSUBSCRIBE_TITLE")?>" href="<?php 
			?><?=$APPLICATION->GetCurPageParam("TOPIC_UNSUBSCRIBE=Y&".bitrix_sessid_get(), array("FORUM_UNSUBSCRIBE", "FORUM_UNSUBSCRIBE_TOPIC", "sessid"))?><?php 
				?>"><?=GetMessage("F_UNSUBSCRIBE")?></a></noindex></span><?php 
		}
	}?>
	</div>
	...
	?><?=trim($arResult["TOPIC"]["TITLE"])?><?php 
		if (strlen($arResult["TOPIC"]["DESCRIPTION"])>0): ?>, <?=trim($arResult["TOPIC"]["DESCRIPTION"])?><?php  endif;
	?></span></div>
</div><?php 
			...
			if ($arParams["SHOW_VOTE"] == "Y" && $res["PARAM1"] == "VT" && intVal($res["PARAM2"]) > 0 && IsModuleInstalled("vote"))
			{
				...
				<div class="forum-info-box forum-post-vote">
					<div class="forum-info-box-inner">
						<span style='position:absolute;'><a style="display:none;" id="message<?=$res["ID"]?>">&nbsp;</a></span><?php  /* IE9 */ ?>
						<a name="message<?=$res["ID"]?>"></a>
						<?php $GLOBALS["APPLICATION"]->IncludeComponent("bitrix:voting.current", $arParams["VOTE_TEMPLATE"],
							array(
								"VOTE_ID" => $res["PARAM2"],
...
<?php 
	if ($arParams["SHOW_RSS"] == "Y")
	{
		?><span class="forum-option-feed"><?php 
			?><noindex><a rel="nofollow" href="<?=$arResult["URL"]["RSS_DEFAULT"]?>" onclick="window.location='<?=CUtil::JSEscape($arResult["URL"]["~RSS"])?>'; return false;">RSS</a></noindex><?php 
		?></span><?php 
	}
	if ($USER->IsAuthorized())
	{
		if ($arParams["SHOW_RSS"] == "Y"): ?>&nbsp;&nbsp;<?php  endif;

		if (empty($arResult["USER"]["SUBSCRIBE"]))
		{
			?><span class="forum-option-subscribe forum-option-do-subscribe"><noindex><a rel="nofollow" title="<?=GetMessage("F_SUBSCRIBE_TITLE")?>" href="<?php 
				?><?=$APPLICATION->GetCurPageParam("TOPIC_SUBSCRIBE=Y&".bitrix_sessid_get(), array("FORUM_SUBSCRIBE", "FORUM_SUBSCRIBE_TOPIC", "sessid"))?><?php 
					?>"><?=GetMessage("F_SUBSCRIBE")?></a></noindex></span><?php 
		}
		else
...
?>
<form class="forum-form" action="<?=POST_FORM_ACTION_URI?>" method="POST" onsubmit="return Validate(this)" <?php 
...
?>
									</select>&nbsp;<input onmousedown="this.form.type.value='messages';this.form.ACTION.value=this.form.ACTION_MESSAGE.value;" <?php 
...
?>
									</select>&nbsp;<input onmousedown="this.form.type.value='topic';this.form.ACTION.value=this.form.ACTION_TOPIC.value;" <?php 
										?>type="submit" value="OK" />
								</div>
<?php 
else:
...
<?php $lastMessage = end($arResult['MESSAGE_LIST']);?>
