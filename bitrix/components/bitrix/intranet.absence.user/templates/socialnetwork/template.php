...
?>
	<div class="bx-user-absence-entry<?php echo $bNow ? ' bx-user-absence-now' : ''?>">
		...
		<span class="bx-user-absence-entry-date"><?php echo GetMessage('INTR_IAU_TPL'.($bNow ? '_TO' : '_FROM'))?> <?php echo FormatDate($DB->DateFormatToPHP(FORMAT_DATETIME), MakeTimeStamp($arEntry['DATE_ACTIVE'.($bNow ? '_TO' : '_FROM')]))?></span>
	</div>
<?php 
	endforeach;
?>
