...
?>
					<td title="<?=htmlspecialcharsbx($tab["title"])?>" id="tab_cont_<?=$tab["id"]?>" class="bx-tab-container<?=($bSelected? "-selected":"")?>" onclick="<?php if(strlen($callback)):?><?=$callback?>('<?=$tab["id"]?>');<?php endif?>bxForm_<?=$arParams["FORM_ID"]?>.SelectTab('<?=$tab["id"]?>');" onmouseover="if(window.bxForm_<?=$arParams["FORM_ID"]?>){bxForm_<?=$arParams["FORM_ID"]?>.HoverTab('<?=$tab["id"]?>', true);}" onmouseout="if(window.bxForm_<?=$arParams["FORM_ID"]?>){bxForm_<?=$arParams["FORM_ID"]?>.HoverTab('<?=$tab["id"]?>', false);}">
						<table cellspacing="0">
							<tr>
...
<a href="javascript:void(0)" onclick="bxForm_<?=$arParams["FORM_ID"]?>.ToggleTabs();" title="<?php echo GetMessage("interface_form_show_all")?>" id="bxForm_<?=$arParams["FORM_ID"]?>_expand_link" class="bx-context-button bx-down"><span></span></a>
<?php endif?>
<?php if($arParams["SHOW_SETTINGS"] || !empty($arThemes)):?>
<a href="javascript:void(0)" onclick="bxForm_<?=$arParams["FORM_ID"]?>.menu.ShowMenu(this, bxForm_<?=$arParams["FORM_ID"]?>.settingsMenu);" title="<?php echo GetMessage("interface_form_settings")?>" class="bx-context-button bx-form-menu"><span></span></a>
<?php endif;?>
					</td>
				</tr>
			</table>
...
<?php if($arParams["~BUTTONS"]["standard_buttons"] !== false):?>
	...
	<input type="submit" name="save" value="<?php echo GetMessage("interface_form_save")?>" title="<?php echo GetMessage("interface_form_save_title")?>" />
	<?php endif?>
	<input type="submit" name="apply" value="<?php echo GetMessage("interface_form_apply")?>" title="<?php echo GetMessage("interface_form_apply_title")?>" />
	<?php if($arParams["BUTTONS"]["back_url"] <> ''):?>
	<input type="button" value="<?php echo GetMessage("interface_form_cancel")?>" name="cancel" onclick="window.location='<?=htmlspecialcharsbx(CUtil::addslashes($arParams["~BUTTONS"]["back_url"]))?>'" title="<?php echo GetMessage("interface_form_cancel_title")?>" />
	<?php endif?>
<?php endif?>
...
<?php if($arResult["OPTIONS"]["expand_tabs"] == "Y"):?>
...
<?php if($bWasRequired):?>
<div class="bx-form-notes"><span class="required">*</span><?php echo GetMessage("interface_form_required")?></div>
<?php endif?>
