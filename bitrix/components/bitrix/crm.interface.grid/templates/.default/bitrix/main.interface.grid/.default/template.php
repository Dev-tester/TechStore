...
<?php endforeach?>
	...
	<tr class="bx-grid-head" oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.settingsMenu"<?php if($GLOBALS['USER']->IsAuthorized()):?> ondblclick="bxGrid_<?=$arParams["GRID_ID"]?>.EditCurrentView()"<?php endif?>>
<?php if($arResult["ALLOW_EDIT"]):?>
...
?>
	<tr<?= $emphasized ? ' class="crm-interface-grid-row-emphasized"' : ''?> oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.oActions[<?=$index?>]"<?php if($sDefAction <> ''):?> ondblclick="<?=$sDefAction?>"<?php endif?> >
<?php if($arResult["ALLOW_EDIT"]):?>
	<?php 
	...
	<tr class="bx-grid-footer">
		<td colspan="<?=$colspan?>">
			...
			<?php foreach($arParams["FOOTER"] as $footer):?>
					<?php $type = isset($footer['type']) ? $footer['type'] : ''?>
					<?php if($type === "row_count"):?>
						<?php $wrapperID = strtolower($arParams["GRID_ID"])."_row_count_wrapper";?>
						<?php $buttonID = strtolower($arParams["GRID_ID"])."_row_count_button";?>
						<td id="<?=$wrapperID?>">
					...
					<?php elseif(isset($footer["custom_html"])):?>
						<?=$footer["custom_html"]?>
					<?php else:?>
		...
		<td colspan="2" align="center">
			<table>
				<tr>
					...
					<td style="background-image:none">
						<div style="margin-bottom:5px"><input type="button" name="up_btn" value="<?php echo GetMessage("interface_grid_view_up")?>" title="<?php echo GetMessage("interface_grid_view_up_title")?>" class="bx-grid-btn" style="width:100px;" disabled onclick="jsSelectUtils.moveOptionsUp(this.form.view_cols)"></div>
						<div style="margin-bottom:5px"><input type="button" name="down_btn" value="<?php echo GetMessage("interface_grid_view_down")?>" title="<?php echo GetMessage("interface_grid_view_down_title")?>" class="bx-grid-btn" style="width:100px;" disabled onclick="jsSelectUtils.moveOptionsDown(this.form.view_cols)"></div>
						<div style="margin-bottom:5px"><input type="button" name="rename_btn" value="<?php echo GetMessage("interface_grid_name_btn")?>" title="<?php echo GetMessage("interface_grid_name_btn_title")?>" class="bx-grid-btn" style="width:100px;" disabled onclick="bxGrid_<?=$arParams["GRID_ID"]?>.RenameColumn()"></div>
					</td>
				</tr>
			</table>
		</td>
...
if ($isAjaxRequest && is_array($arParams['~GRID_INIT_EVENT_PARAMS']))
{
	?>
bxGrid_<?=$arParams["GRID_ID"]?>.initEventParams = <?=CUtil::PhpToJSObject($arParams['~GRID_INIT_EVENT_PARAMS'])?>;<?php 
}
?>
</script>
