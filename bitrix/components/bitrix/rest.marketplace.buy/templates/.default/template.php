...
?>
				<span <?php if ($app["ACTIVE"] == "Y"):?>style="display:none"<?php endif?> id="mp_uninstalled_block_<?=$app["CODE"]?>">
<?php
...
<?php
			if ($app["STATUS"] == "P"):
?>
					<a href="javascript:void(0)" class="bt_green" onclick="BX.rest.Marketplace.install(<?php echo CUtil::PhpToJSObject($arParamsApp)?>);"><?=GetMessage("MARKETPLACE_INSTALL_BUTTON")?></a>
<?php
			else:
				if ($arResult["ITEMS"][$app["CODE"]]["DEMO"] == "D"):
?>
					<a href="javascript:void(0)" class="bt_green" onclick="BX.rest.Marketplace.install(<?php echo CUtil::PhpToJSObject($arParamsApp)?>);"><?=GetMessage("MARKETPLACE_APP_DEMO")?></a>
<?php
				elseif ($arResult["ITEMS"][$app["CODE"]]["DEMO"] == "T" && ($app["IS_TRIALED"] == "N" || MakeTimeStamp($app["DATE_FINISH"]) > time())):
?>
					<a href="javascript:void(0)" class="bt_green" onclick="BX.rest.Marketplace.install(<?php echo CUtil::PhpToJSObject($arParamsApp)?>);"><?=GetMessage("MARKETPLACE_APP_TRIAL")?></a>
<?php
				endif;
			endif;
		else:
?>
					<a href="javascript:void(0)" onclick="BX.rest.Marketplace.install(<?php echo CUtil::PhpToJSObject($arParamsApp)?>);" class="bt_green" ><?=GetMessage("MARKETPLACE_INSTALL_BUTTON")?></a>
<?php
		endif;
?>
				</span>
