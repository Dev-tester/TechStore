...
if($arParams['arUserField']["SETTINGS"]["DISPLAY"] != "CHECKBOX")
{
	...
	{
		?>
		<select multiple="multiple" name="<?php echo $arParams["arUserField"]["FIELD_NAME"]?>[]" style="width: 225px;"<?php echo $listLength > 1 ? ' size="'.$listLength.'"' : ''?>>
	...
	{
		?>
		<select name="<?php echo $arParams["arUserField"]["FIELD_NAME"]?>" style="width: 225px;"<?php echo $listLength > 1 ? ' size="'.$listLength.'"' : ''?>>
		<option value=""><?php echo GetMessage("MAIN_NO")?></option>
		<?php 
		$bWasSelect = false;
		foreach ($arParams["arUserField"]["USER_TYPE"]["FIELDS"] as $key => $val)
