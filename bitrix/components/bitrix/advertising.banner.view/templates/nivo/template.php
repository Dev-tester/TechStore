...
<?php  if ($arParams['CASUAL_PROPERTIES']['TYPE'] == 'template'): ?>
	<?php  if (isset($arParams['FILES']['IMG']['SRC'])): ?>
		...
		<?php  endif ?>
		<img <?php  if ($arParams['PROPS']['EFFECT']!=''){echo 'data-transition="'.$arParams['PROPS']['EFFECT'].'"';}?> src="<?=$arParams['FILES']['IMG']['SRC']?>" alt="<?=$arParams['FILES']['IMG']['DESCRIPTION']?>" title="#htmlcaption<?=$rnd?>">
		<?php  if ($arParams['PROPS']['LINK_URL'] != '' && !isset($arParams['PREVIEW'])): ?>
			</a>
		<?php  endif ?>
		<div id="htmlcaption<?=$rnd?>" style="display:none">
			<?php  if ($arParams['EXT_MODE'] == 'N'): ?>
				...
				<div class="bx-slider-preset-<?=$arParams['PROPS']['PRESET']?>">
					<div class="bx-advertisingbanner-content" <?php  if ($arParams['PROPS']['PRESET']==2 || $arParams['PROPS']['PRESET']==3){echo 'style="background:rgba('.$arParams['PROPS']['HEADING_BG_COLOR'].');"';}?>>
					<?php  if ($arParams['PROPS']['HEADING_SHOW'] == 'Y'): ?>
						<div id="slider-caption-<?=$rnd?>" class="bx-advertisingbanner-text-title" style="font-size:<?=$arParams['PROPS']['HEADING_FONT_SIZE']?>px;color:#<?=$arParams['PROPS']['HEADING_FONT_COLOR']?>;<?php  if ($arParams['PROPS']['PRESET']==1 || $arParams['PROPS']['PRESET']==4){echo 'background:rgba('.$arParams['PROPS']['HEADING_BG_COLOR'].');';}?>">
							<?=$headingText?>
						</div>
					<?php  endif ?>
...
<?php  else: ?>
	<?php  if (isset($arParams['FILES']['CASUAL_IMG']['SRC'])): ?>
		<?php  if ($arParams['CASUAL_PROPERTIES']['URL'] != ''): ?>
