...
#COMMENTS_BEGIN#
<?php foreach ($arResult["MESSAGES_REV"] as $res):?>
	<?php if ($arParams['FETCH_USER_ALIAS'])
	...
	<div class="photo-comment" id="bxphoto_com_<?=$res["ID"]?>">
		<div class="photo-comment-avatar <?php if ($res['AUTHOR_PHOTO'] == ''){echo 'photo-comment-avatar-none';}?>" >
			<?php if ($res['AUTHOR_PHOTO'] != ''):?>
		...
		<div onmouseout="BX.removeClass(this, 'photo-comment-hover')" onmouseover="BX.addClass(this, 'photo-comment-hover')" class="photo-comment-info-text">
			<div class="photo-comment-info">
				...
				<?php if (intVal($res["AUTHOR_ID"]) > 0 && !empty($res["AUTHOR_URL"])):?>
				...
				<span class="review-rating rating_vote_text">
					...
					// mantis:48973
					?><script type="text/javascript">if (!window.BX && top.BX){window.BX = top.BX;}</script><?php 
...
#ADD_COMMENT_BEGIN#
<?php if($arResult["USER"]["PERMISSION"] > "E"): /* USER CAN WRITE MESSAGES*/?>
...
<?php if (!$arResult["IS_AUTHORIZED"]): /* GUEST PANEL */?>
	<div class="reviews-reply-fields">
		<div class="reviews-reply-field-user">
			<div class="reviews-reply-field reviews-reply-field-author">
				<input name="REVIEW_AUTHOR" id="REVIEW_AUTHOR<?=$arParams["form_index"]?>" size="30" type="text" value="<?= ($arResult["REVIEW_AUTHOR"] != '' ? $arResult["REVIEW_AUTHOR"] : GetMessage("OPINIONS_NAME").'...')?>" title="<?=GetMessage("OPINIONS_NAME")?>" <?php if($arResult["REVIEW_AUTHOR"] != ''){echo 'class="bxph-photo-active"';}?> />
			</div>
<?php if ($arResult["FORUM"]["ASK_GUEST_EMAIL"]=="Y"):?>
			<div class="reviews-reply-field reviews-reply-field-email"><?php /* <label for="REVIEW_EMAIL<?=$arParams["form_index"]?>"><?=GetMessage("OPINIONS_EMAIL")?></label>*/?>
				<input type="text" name="REVIEW_EMAIL" id="REVIEW_EMAIL<?=$arParams["form_index"]?>" size="30" value="<?= ($arResult["REVIEW_EMAIL"] != '' ? $arResult["REVIEW_EMAIL"] : GetMessage("OPINIONS_EMAIL").'...')?>" title="<?=GetMessage("OPINIONS_EMAIL")?>" <?php if($arResult["REVIEW_EMAIL"] != ''){echo 'class="bxph-photo-active"';}?>/>
			</div>
<?php endif;?>
			<div class="reviews-clear-float"></div>
		</div>
	</div>
<?php endif; /* if (!$arResult["IS_AUTHORIZED"]) */?>

	<div class="reviews-reply-fields">
		<div class="reviews-reply-field reviews-reply-field-text">
			<textarea class="photo-textarea" name="REVIEW_TEXT" id="REVIEW_TEXT"><?=$arResult["REVIEW_TEXT"];?></textarea>
		</div>
<?php if (strLen($arResult["CAPTCHA_CODE"]) > 0): /* CAPTHCA */?>
...
<?php endif; /* if($arResult["USER"]["PERMISSION"] > "E") */?>
#ADD_COMMENT_END#
