	...
	<div class="subscription-form">
		...
		<table cellspacing="0" class="subscription-layout">
			<tr>
				...
				<td class="field-form">
					<div class="subscription-format"><span><?php echo GetMessage("CT_BSE_FORMAT_LABEL")?></span>&nbsp;<input type="radio" name="FORMAT" id="MAIL_TYPE_TEXT" value="text" <?php if($arResult["SUBSCRIPTION"]["FORMAT"] != "html") echo "checked"?> /><label for="MAIL_TYPE_TEXT"><?php echo GetMessage("CT_BSE_FORMAT_TEXT")?></label>&nbsp;<input type="radio" name="FORMAT" id="MAIL_TYPE_HTML" value="html" <?php if($arResult["SUBSCRIPTION"]["FORMAT"] == "html") echo "checked"?> /><label for="MAIL_TYPE_HTML"><?php echo GetMessage("CT_BSE_FORMAT_HTML")?></label></div>
				...
				<td class="field-form">
					<?php foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
						<div class="subscription-rubric">
							<input type="checkbox" id="RUBRIC_<?php echo $itemID?>" name="RUB_ID[]" value="<?=$itemValue["ID"]?>"<?php if($itemValue["CHECKED"]) echo " checked"?> /><label for="RUBRIC_<?php echo $itemID?>"><b><?php echo $itemValue["NAME"]?></b><span><?php echo $itemValue["DESCRIPTION"]?></span></label>
						</div>
					<?php endforeach;?>
