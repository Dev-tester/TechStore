...
else
{
	...
	<tr>
		<td valign="top" width="65%">
			...
			<?php if (false && $GLOBALS["USER"]->IsAuthorized()):?>
				...
				<?php endif;?>
				<?php if (($arResult["CurrentUserPerms"]["UserCanInitiate"] || !$arResult["CurrentUserPerms"]["UserRole"] || $arResult["CurrentUserPerms"]["UserIsMember"] && !$arResult["CurrentUserPerms"]["UserIsOwner"])):?>
				<div class="bx-group-control">
					<ul>
					<?php if ($arResult["CurrentUserPerms"]["UserCanInitiate"] && $arResult["Group"]["OPENED"] != "Y"):?>
						<li class="bx-icon-requests"><a href="<?= $arResult["Urls"]["GroupRequests"] ?>"><?= GetMessage("SONET_C6_ACT_VREQU") ?></a></li>
					<?php endif;?>
					<?php if (!$arResult["CurrentUserPerms"]["UserRole"]):?>
