...
<form action="<?=POST_FORM_ACTION_URI?>" name="crmPermForm" method="POST">
	...
	<table width="100%" cellpadding="0" cellspacing="0" class="crmPermRoleTable" id="crmPermRoleTable" >
		...
		<?php  foreach ($arResult['ENTITY'] as $entityType => $entityName): ?>
		<tr>
			<td><?php  if (isset($arResult['ENTITY_FIELDS'][$entityType])): ?><a href="javascript:void(0)" class="crmPermRoleTreePlus" onclick="CrmPermRoleShowRow(this)"></a><?php endif;?><?=$entityName?></td>
			<td>
				<?php  if (in_array('READ', $arResult['ENTITY_PERMS'][$entityType])): ?>
				...
				<span id="divPermsBox<?=$entityType?>Read_Select" style="display:none">
					...
					<?php  foreach ($arResult['ROLE_PERM'][$entityType] as $rolePermAtr => $rolePermName): ?>
			...
			<td>
				<?php  if (in_array('ADD', $arResult['ENTITY_PERMS'][$entityType])): ?>
			...
			<td>
				<?php  if (in_array('WRITE', $arResult['ENTITY_PERMS'][$entityType])):
			...
			<td>
				<?php  if (in_array('DELETE', $arResult['ENTITY_PERMS'][$entityType])): ?>
			...
			<td>
				<?php  if (in_array('EXPORT', $arResult['ENTITY_PERMS'][$entityType])): ?>
			...
			<td>
				<?php  if (in_array('IMPORT', $arResult['ENTITY_PERMS'][$entityType])): ?>
			...
			<td>
				<?php if (strpos($entityType, 'LEAD') === 0 || strpos($entityType, 'DEAL') === 0 || strpos($entityType, 'ORDER') === 0):
					$curVal = $canWriteConfig ? BX_CRM_PERM_ALL : $arResult['ROLE_PERMS'][$entityType]['AUTOMATION']['-'];
				?>
					<span id="divPermsBox<?=$entityType?>Automation" data-role="automation-perm" class="divPermsBoxText <?php if ($canWriteConfig):?>divPermsBoxTextDisabled<?php endif?>" onclick="if (!BX.hasClass(this, 'divPermsBoxTextDisabled')){CrmPermRoleShowBox(this.id);}"><?=$arResult['ROLE_PERM']['AUTOMATION'][$curVal]?></span>
					...
					<select id="divPermsBox<?=$entityType?>Automation_SelectBox" name="ROLE_PERMS[<?=$entityType?>][AUTOMATION][-]">
						<?php  foreach ($arResult['ROLE_PERM']['AUTOMATION'] as $rolePermAtr => $rolePermName): ?>
		...
		<?php 	if (isset($arResult['ENTITY_FIELDS'][$entityType])):
