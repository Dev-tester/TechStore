...
?><div id="disk-attach-block-<?=$arResult['UID']?>" class="feed-com-files diskuf-files-entity"><?php 
	if (!empty($arResult['IMAGES']))
	{
		...
		?><div class="feed-com-files">
			...
			<div class="feed-com-files-cont"><?php 
				...
				{
					?><span class="feed-com-files-photo feed-com-files-photo-load" id="disk-attach-<?=$file['ID']?>"<?php 
						?> style="width:<?=$file["THUMB"]["width"]?>px;height:<?=$file["THUMB"]["height"]?>px;"><?php 
						...
						?><img id="<?=$id?>" onload="this.parentNode.className='feed-com-files-photo';" <?php 
						if (
							isset($arParams["LAZYLOAD"]) 
							&& $arParams["LAZYLOAD"] == "Y"
						)
						{
							...
							?> data-thumb-src="<?=$file["THUMB"]["src"] ?>"<?php 
						}
						else
						{
							?> src="<?=$file["THUMB"]["src"] ?>" <?php 
						}
						?> width="<?=$file["THUMB"]["width"]?>"<?php 
						?> height="<?=$file["THUMB"]["height"]?>"<?php 
						?> border="0"<?php 
						?> alt="<?=htmlspecialcharsbx($file["NAME"])?>"<?php 
						?> <?=$file['ATTRIBUTES_FOR_VIEWER']?> <?php 
		...
		if (strlen($jsIds) > 0)
		{
			?><script>BX.LazyLoad.registerImages([<?=$jsIds?>], null, {dataSrcName: "thumbSrc"});</script><?php 
		}
	}
		...
		foreach($arResult['FILES'] as $file)
		{
			...
			?><div class="<?=$className?>">
				...
				<span class="feed-com-file-name-wrap">
					<a <?= ($file['FROM_EXTERNAL_SYSTEM'] && $file['CAN_UPDATE'])? 'style="color:#d9930a;"' : '' ?> target="_blank" href="<?=htmlspecialcharsbx($file['DOWNLOAD_URL'])?>"<?php 
						?> id="disk-attach-<?=$file['ID']?>"<?php 
						?> class="feed-com-file-name" <?php 
						?> title="<?=htmlspecialcharsbx($file['NAME'])?>" <?php 
						?> bx-attach-file-id="<?=$file['FILE_ID']?>"<?php 
						if ($file['XML_ID']): ?> bx-attach-xml-id="<?=$file['XML_ID']?>"<?php endif;
						if ($file['TYPE_FILE']): ?> bx-attach-file-type="<?=$file['TYPE_FILE']?>"<?php endif;
						?> data-bx-baseElementId="disk-attach-<?=$file['ID']?>" <?=
							$file['ATTRIBUTES_FOR_VIEWER']
						?> alt="<?=htmlspecialcharsbx($file['NAME'])?>"<?php 
					?>><?=htmlspecialcharsbx($file['NAME'])?><?php 
					?></a><?php 
					...
					?><script type="text/javascript">
						...
						BX.Disk.Files['<?= $file['ID'] ?>'] = [
							...
							<?php  if($file['EDITABLE'] && $file['CAN_UPDATE'] && (!$file['IS_LOCKED'] || $file['IS_LOCKED_BY_SELF']) && !$arParams['DISABLE_LOCAL_EDIT']){ ?>
							...
							<?php  } ?>
							<?php  if($file['FROM_EXTERNAL_SYSTEM'] && $file['CAN_UPDATE'] && (!$file['IS_LOCKED'] || $file['IS_LOCKED_BY_SELF'])){ ?>
					...
					if($file['EDITABLE'] && $file['CAN_UPDATE'] && (!$file['IS_LOCKED'] || $file['IS_LOCKED_BY_SELF']) && !$arParams['DISABLE_LOCAL_EDIT']) {
						?><a class="feed-con-file-changes-link" href="#" onclick="top.BX.UI.Viewer.Instance.runActionByNode(BX('disk-attach-<?=$file['ID']?>'), 'edit', {
							modalWindow: BX.Disk.openBlankDocumentPopup()
						}); return false;"><?= GetMessage('WDUF_FILE_EDIT') ?></a><?php 
					}
		...
		foreach($arResult['DELETED_FILES'] as $file)
		{
			?><div class="<?=$className?>">
				...
				<span class="feed-com-file-name-wrap">
					<span <?php 
						?> id="disk-attach-<?=$file['ID']?>"<?php 
						?> class="feed-com-file-deleted-name" <?php 
						?> title="<?=htmlspecialcharsbx($file['NAME'])?>" <?php 
					?>><?=htmlspecialcharsbx($file['NAME'])?><?php 
					?></span><?php 
					?><span class="feed-com-file-size"><?=$file['SIZE']?></span>
					<span class="feed-con-file-text-notice" href="#"><?= GetMessage('DISK_UF_FILE_IS_DELETED') ?></span><?php 
					if($file['CAN_RESTORE'] && $file['TRASHCAN_URL']) {
						?><a class="feed-con-file-changes-link" href="<?= $file['TRASHCAN_URL'] ?>"><?= GetMessage('DISK_UF_FILE_RESTORE') ?></a><?php 
					} ?>
				</span>
			</div><?php 
		}
