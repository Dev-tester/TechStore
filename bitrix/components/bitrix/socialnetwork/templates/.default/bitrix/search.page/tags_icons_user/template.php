...
<form action="" method="get" name="sonet_content_search_form" id="sonet_content_search_form">
	...
	<?php if(
		...
		?><div class="sonet-search-advanced"><div class="sonet-search-advanced-filter"><a href="#" onclick="document.getElementById('sonet_content_search_filter').style.display = document.getElementById('sonet_content_search_filter').style.display == 'none' ? 'block' : 'none'; return false;"><?=GetMessage("SEARCH_ADDITIONAL_FILTER")?></a></div></div><?php 
		...
		if(array_key_exists("DROPDOWN_SONET", $arResult) && is_array($arResult["DROPDOWN_SONET"]) && count($arResult["DROPDOWN_SONET"]) > 0):?>
			<tr>
				...
				<td class="sonet-search-filter-field"><select name="<?=$arParams["FILTER_NAME"]?>" class="select-field">
					<option value=""><?=GetMessage("SEARCH_ALL")?></option>
					<?php foreach($arResult["DROPDOWN_SONET"] as $key=>$value):?>
						<option value="<?=$key?>"<?php if($GLOBALS[$arParams["FILTER_NAME"]]["SONET_FEATURE"]==$key) echo " selected"?>><?=$value?></option>
					<?php endforeach?>
				</select></td>
			</tr>
		<?php endif;
		...
		if(array_key_exists("FILTER_DATE_NAME", $arParams) && strlen($arParams["FILTER_DATE_NAME"]) > 0):
			...
			<tr>
				...
				<td class="sonet-search-filter-field"><?php 
...
<?php elseif(count($arResult["SEARCH"])>0):?>
	...
	<?php foreach($arResult["SEARCH"] as $arItem):?>
		...
		<?php endif;?>
		<small><?=GetMessage("SEARCH_MODIFIED")?> <?=$arItem["DATE_CHANGE"]?><br /></small><?php 
