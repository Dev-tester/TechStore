...
<?=bitrix_sessid_post();?>
<?php endif?>
<table cellspacing="0" class="bx-disk-interface-grid<?php if($arResult["OPTIONS"]["theme"] <> '') echo " bx-disk-interface-grid-theme-".$arResult["OPTIONS"]["theme"]?>" id="<?=$arParams["GRID_ID"]?>">
	<thead>
	<tr class="bx-disk-grid-gutter" <?php if(empty($arParams['DISABLE_SETTINGS'])) {?>oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.settingsMenu<?php  } ?>">
...
<?php endforeach?>
	...
	<tr class="bx-disk bx-disk-grid-head" <?php if(empty($arParams['DISABLE_SETTINGS'])) {?>oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.settingsMenu"<?php if($USER->IsAuthorized()):?> ondblclick="bxGrid_<?=$arParams["GRID_ID"]?>.EditCurrentView()"<?php endif?><?php  } ?>>
<?php if($arResult["ALLOW_EDIT"] || $arParams["ALLOW_EDIT"]):?>
		<td class="bx-disk-checkbox-col" width="1%"><input type="checkbox" name="" id="<?=$arParams["GRID_ID"]?>_check_all" value="" title="<?php echo GetMessage("interface_grid_check_all")?>" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.SelectAllRows(this);"></td>
<?php endif?>
		<td class="bx-disk-actions-col" width="1%"><a href="javascript:void(0);"
			...
			title="<?php echo GetMessage("interface_grid_settings")?>" class="bx-disk-action"></a></td>
<?php 
...
?>
	<tr class="bx-disk-table-body" oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.oActions[<?=$index?>]"<?php if($sDefAction <> ''):?> ondblclick="<?=htmlspecialcharsbx($sDefAction)?>" title="<?=GetMessage("interface_grid_dblclick")?><?=$sDefTitle?>"<?php endif?>>
<?php if($arResult["ALLOW_EDIT"] || $arParams["ALLOW_EDIT"]):?>
	<?php 
	...
	?>
		<td class="bx-disk-checkbox-col"><input type="checkbox" name="ID[]" id="ID_<?=$data_id?>" value="<?=$data_id?>" title="<?php echo GetMessage("interface_grid_check")?>"></td>
	<?php else:?>
...
<?php endif?>
	<?php if(is_array($aRow["actions"]) && count($aRow["actions"]) > 0):?>
		<td class="bx-disk-actions-col"><a href="javascript:void(0);"
			...
			title="<?php echo GetMessage("interface_grid_act")?>" class="bx-disk-action"></a></td>
	<?php else:?>
		<td>&nbsp;</td>
	<?php endif?>
<?php foreach($arResult["HEADERS"] as $id=>$header):?>
		<td<?=(isset($header["sort_state"]) && $header["sort_state"] <> ''? ' class="bx-disk-sorted"':'')?><?php 
...
?>
	<tr><td class="bx-disk-not-result" colspan="<?=$colspan?>"><?php echo GetMessage("interface_grid_no_data_2")?></td></tr>
<?php endif?>
</tbody>
<?php if($arResult["ALLOW_EDIT"] || $arParams["ALLOW_EDIT"] || is_array($arParams["FOOTER"]) && count($arParams["FOOTER"]) > 0 || !empty($arResult["NAV_STRING"])):?>
	<tfoot>
	<tr>
		<td colspan="<?=$colspan?>">
			...
			<?php foreach($arParams["FOOTER"] as $footer):?>
				<?php  if(!empty($footer['custom_html'])){ ?>
					<?= $footer['custom_html'] ?>
				<?php  } elseif(!empty($footer['place_for_pagination']) && !empty($arResult["NAV_STRING"])){ ?>
					<td class="tar bx-disk-nav-string-cont"><?= $arResult["NAV_STRING"] ?></td>
				<?php  } elseif(!empty($footer["title"])) {?>
...
<?php if($arParams["ALLOW_EDIT"] || $arResult["ALLOW_EDIT"]):?>
...
<!--				<tr>-->
		<td class="vam" style="display:none" id="bx_grid_<?=$arParams["GRID_ID"]?>_action_buttons">
			<input type="submit" name="save" value="<?php echo GetMessage("interface_grid_save")?>" title="<?php echo GetMessage("interface_grid_save_title")?>" class="bx-disk-btn bx-disk-btn-medium bx-disk-btn-green">
			<input type="button" name="" value="<?php echo GetMessage("interface_grid_cancel")?>" title="<?php echo GetMessage("interface_grid_cancel_title")?>" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ActionCancel();" class="bx-disk-btn bx-disk-btn-medium bx-disk-btn-lightgray">
		</td>
...
<?php endif?>
<?php if($arParams["ALLOW_INLINE_EDIT"] || $arResult["ALLOW_INLINE_EDIT"]):?>
	<?php if($bNeedSep):?>
<!--		<td><div class="bx-disk-separator"></div></td>-->
	<?php endif;?>
		<td class="vam"><a href="javascript:void(0);" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ActionEdit(this);" title="<?php echo GetMessage("interface_grid_edit_selected")?>" class="bx-disk-bd-icon-edit" id="edit_button_<?=$arParams["GRID_ID"]?>"></a></td>
<?php 
	$bNeedSep = true;
endif;
?>
<?php if($arParams["ACTIONS"]["delete"] == true):?>
	<?php if($bNeedSep && !$arResult["ALLOW_INLINE_EDIT"]):?>
		<td><div class="bx-disk-separator"></div></td>
	<?php endif?>
		<td class="vam"><a href="javascript:void(0);" title="<?php echo GetMessage("interface_grid_delete_title")?>" class="bx-disk-bd-icon-del" id="delete_button_<?=$arParams["GRID_ID"]?>"></a></td>
<?php 
	$bNeedSep = true;
endif;
?>
...
if($arParams["~ACTIONS"]["custom_html"] <> ''):
	$bShowApply = true;
?>
	<?php if(!empty($arParams["~ACTIONS"]["before_custom_html"])):?>
		<td class="vam"><?= $arParams["~ACTIONS"]["before_custom_html"] ?></td>
	<?php endif?>
		<td class="vam"><?=$arParams["~ACTIONS"]["custom_html"]?></td>
<?php endif?>
<?php if($bShowApply):?>
		<td class="vam" style="padding-left:2px;"><input class="bx-disk-btn bx-disk-btn-medium bx-disk-btn-gray mb0" type="submit" name="apply" value="<?php echo GetMessage("interface_grid_apply")?>" disabled></td>
<?php endif?>
<!--				</tr>-->
<!--			</table>-->
<!--		</td>-->
...
<?php if($arResult["IS_ADMIN"]):?>
	...
	<tr>
		<td colspan="2"><input type="checkbox" name="set_default_settings" id="set_default_settings_<?=$arParams["GRID_ID"]?>" onclick="BX('delete_users_settings_<?=$arParams["GRID_ID"]?>').disabled = !this.checked;"><label for="set_default_settings_<?=$arParams["FORM_ID"]?>"><?php echo GetMessage("interface_grid_common_default")?></label></td>
	</tr>
	<tr>
...
?>
				...
				<span class="bx-filter-days" style="display:none"><input type="text" name="<?=$field["id"]."_days"?>" value="" size="5" /> <?php echo GetMessage("inerface_grid_days")?></span>
				<span class="bx-filter-from" style="display:none"><input type="text" name="<?=$field["id"]."_from"?>" value=""<?=$params?> /><?php 
$APPLICATION->IncludeComponent(
	...
	$component,
	array("HIDE_ICONS"=>true)
);?></span><span class="bx-filter-hellip" style="display:none">&hellip;</span><span class="bx-filter-to" style="display:none"><input type="text" name="<?=$field["id"]."_to"?>" value=""<?=$params?> /><?php 
$APPLICATION->IncludeComponent(
	"bitrix:main.calendar",
	"",
	array(
