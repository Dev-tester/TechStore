...
?>
		<div class="bx-disk-file-container" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.clickAndSelectRow(<?= $aRow["data"]["ID"] ?>, event);" oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.oActions[<?=$index?>]"<?php if($sDefAction <> ''):?> ondblclick="<?=htmlspecialcharsbx($sDefAction)?>" title="<?=GetMessage("interface_grid_dblclick")?><?=$sDefTitle?>"<?php endif?>>
			<div class="draggable bx-disk-file-block" data-object-id="<?= (!empty($aRow["id"])? $aRow["id"] : $aRow["data"]["ID"]) ?>">
				...
				<?php if($arResult["ALLOW_EDIT"]):?>
					...
					?>
						<div class="bx-disk-checkbox-col bx-disk-checkbox-container">
							<input type="checkbox" name="ID[]" id="ID_<?=$data_id?>" value="<?=$data_id?>" title="<?php echo GetMessage("interface_grid_check")?>">
				...
				<?php if(is_array($aRow["actions"]) && count($aRow["actions"]) > 0):?>
					<a href="javascript:void(0);" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ShowActionMenu(this, <?=$index?>);" title="<?php echo GetMessage("interface_grid_act")?>" class="bx-disk-action"></a>
				<?php endif?>
				<?php if(!empty($shareInfoAction)):?>
...
?>
	<div class="bx-disk-not-result"><?php echo GetMessage("interface_grid_no_data_2")?></div>
<?php endif?>
		<div class="clb"></div>
	</div>
	...
	<table cellspacing="0" class="bx-disk-interface-grid" id="" dropzone="">
		<tbody>
			<?php if($arResult["ALLOW_EDIT"] || is_array($arParams["FOOTER"]) && count($arParams["FOOTER"]) > 0 || !empty($arResult["NAV_STRING"])):?>
...
<!--		<td><div class="bx-disk-separator"></div></td>-->
	<?php endif;?>
		<td><a href="javascript:void(0);" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ActionEdit(this);" title="<?php echo GetMessage("interface_grid_edit_selected")?>" class="bx-disk-bd-icon-edit" id="edit_button_<?=$arParams["GRID_ID"]?>"></a></td>
<?php 
	$bNeedSep = true;
endif;
?>
<?php if($arParams["ACTIONS"]["delete"] == true):?>
	<?php if($bNeedSep && !$arResult["ALLOW_INLINE_EDIT"]):?>
		<td><div class="bx-disk-separator"></div></td>
	<?php endif?>
		<td><a href="javascript:void(0);" title="<?php echo GetMessage("interface_grid_delete_title")?>" class="bx-disk-bd-icon-del" id="delete_button_<?=$arParams["GRID_ID"]?>"></a></td>
<?php 
	$bNeedSep = true;
endif;
?>
