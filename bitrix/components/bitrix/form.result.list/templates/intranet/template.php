...
?>
<p>
<b><a href="<?=$arParams["NEW_URL"]?><?=$arParams["SEF_MODE"] != "Y" ? (strpos($arParams["NEW_URL"], "?") === false ? "?" : "&")."WEB_FORM_ID=".$arParams["WEB_FORM_ID"] : ""?>"><?php echo GetMessage('FORM_ADD')?>&nbsp;&nbsp;&gt;&gt;</a></b>
</p>
				...
				<th>
					<table class="form-results-header-inline">
						<tr>
							...
							?>ID<?php if ($arParams["SHOW_STATUS"]!="Y") { ?><br /><?=SortingEx("s_id")?><?php  } //endif($SHOW_STATUS!="Y");?></th>
							<?php 
							if ($arParams["SHOW_STATUS"]=="Y")
							{
							?>
...
?>
						<input type="hidden" name="RESULT_ID[]" value="<?=$arRes["ID"]?>" /><b><?php echo $href ? '<a href="'.$href.'">' : ''?><?php echo GetMessage('FORM_RESULT_TITLE')?><span class="form-result-id"><?php echo $arRes["ID"]?></span><?php echo $href ? '</a>' : ''?></b><br />
<?php 
				if ($arParams["SHOW_STATUS"] == "Y")
				{
?>
...
?>
						<div class="form-result-controls"><?php echo implode('&nbsp;|&nbsp;', $arControls)?></div>
<?php 
?>
</div>
				</td>
				...
				<td>
					...
					if (is_array($arrAnswer))
					{
						...
						{
						?>
							<?php if (strlen(trim($arrA["USER_TEXT"])) > 0) {?><?=$arrA["USER_TEXT"]?><br /><?php }?>
							<?php if (strlen(trim($arrA["ANSWER_TEXT"])) > 0) {?><span class='form-anstext'><?=$arrA["ANSWER_TEXT"]?></span>&nbsp;<?php }?>
							<?php if (strlen(trim($arrA["ANSWER_VALUE"])) > 0 && $arParams["SHOW_ANSWER_VALUE"]=="Y") {?>(<span class='form-ansvalue'><?=$arrA["ANSWER_VALUE"]?></span>)<?php }?>
									<br />
									<?php 
