...
<?php include($_SERVER["DOCUMENT_ROOT"].$templateFolder."/script.php");
	...
	<div id="form_comment_" >
		...
		<div class="blog-comment-fields">
			...
			<div class="blog-comment-buttons">
								<?php if($arResult["IDEA_MODERATOR"]):?>
			...
			{?>
				...
				<div class="idea-answer idea-comment-rating-content-<?=$arParams['RATING_TEMPLATE']?>" style="padding-left:5px;">
					...
					elseif(strlen($comment["urlToAuthor"])>0)
					{
						?>
							<noindex>
						<?php $GLOBALS["APPLICATION"]->IncludeComponent("bitrix:main.user.link",
							'',
							array(
								"ID" => $comment["arUser"]["ID"],
								"HTML_ID" => "blog_post_comment_".$comment["arUser"]["ID"],
				...
				<div class="blog-comment-content-common">
					...
					<div class="blog-post-meta<?php if((strlen($comment["urlToShow"])==0 || strlen($comment["urlToHide"])==0) && $comment["CAN_EDIT"] != "Y" && strlen($comment["urlToDelete"])==0):?> blog-post-meta-empty<?php endif?>">
					<?php 
