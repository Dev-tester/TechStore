...
if($arParams['arUserField']["SETTINGS"]["DISPLAY"] != "CHECKBOX")
{
	...
	{
		?>
		<select multiple="multiple" name="<?php echo $arParams["arUserField"]["FIELD_NAME"]?>" size="<?php echo $arParams["arUserField"]["SETTINGS"]["LIST_HEIGHT"]?>" class="filter-dropdown">
		<?php 
	...
	{
		?>
		<select name="<?php echo $arParams["arUserField"]["FIELD_NAME"]?>" size="<?php echo $arParams["arUserField"]["SETTINGS"]["LIST_HEIGHT"]?>" class="filter-dropdown">
		<?php 
		$bWasSelect = false;
		foreach ($arParams["arUserField"]["USER_TYPE"]["FIELDS"] as $key => $val)
		{
