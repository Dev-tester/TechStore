...
<div class="user-profile-right-block">			
	...
	foreach ($arResult['ENTRIES'] as $arEntry):
		...
		<?php echo GetMessage('INTR_IAU_TPL'.($bNow ? '_TO' : '_FROM'))?> <?php echo FormatDate($DB->DateFormatToPHP(FORMAT_DATETIME), MakeTimeStamp($arEntry['DATE_ACTIVE'.($bNow ? '_TO' : '_FROM')]))?><br>
		<?php echo htmlspecialcharsbx($arEntry['TITLE'])?>
		</div><?php 
		$bFirst = false;
	endforeach;
	?>	
