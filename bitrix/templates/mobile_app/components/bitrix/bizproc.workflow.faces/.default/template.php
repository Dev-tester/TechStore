...
<div class="bp-short-process">
	...
	<?php endif?>
	<div class="bp-short-process-steps <?php if (empty($arResult['TASKS']['COMPLETED']) && !$arResult['STATE_TITLE']) echo 'alone';?>">
		<div class="bp-short-process-step-wrapper">
			<a href="<?=empty($arResult['STARTED_BY'])? 'javascript:void(0)' : SITE_DIR.'mobile/users/?user_id='.(int)$arResult['STARTED_BY']['ID']?>" class="bp-short-process-step bp-short-process-step-firs">
				<span class="bp-short-process-step-inner">
					<?php if (!empty($arResult['STARTED_BY']) && is_array($arResult['STARTED_BY'])):
						$startedPhoto = CBPViewHelper::getUserPhotoSrc($arResult['STARTED_BY']);
						if (!$startedPhoto)
							$startedPhoto = $defaultAvatar;
						?>
						<img src="<?=$startedPhoto?>" border="0"/>
					<?php elseif (!empty($arResult['DOCUMENT_ID']) && in_array($arResult['DOCUMENT_ID'][0], array('crm', 'disk', 'lists', 'tasks'))):?>
		...
		<?php if (!empty($arResult['TASKS']['COMPLETED'][0])):
			...
			<div class="bp-short-process-step-wrapper <?php if ($task['USERS_CNT'] > 1) echo 'bp-short-process-step-wrapper-more'?>">
				<a href="<?=SITE_DIR.'mobile/users/?user_id='.(int)$face['USER_ID']?>" class="bp-short-process-step <?php if ($face['STATUS'] == CBPTaskUserStatus::Ok || $face['STATUS'] == CBPTaskUserStatus::Yes) echo 'bp-short-process-step-ready'?>
			<?php if ($face['STATUS'] == CBPTaskUserStatus::No || $face['STATUS'] == 4) echo 'bp-short-process-step-cancel'?> <?php if ($task['USERS_CNT'] > 1) echo 'bp-short-process-step-more'?>">
	...
	<?php if (!empty($arResult['TASKS']['RUNNING'][0])):
			...
			if (!$photoSrc)
				$photoSrc = $defaultAvatar
		?>
		<span class="bp-short-prosess-steps-arrow <?php if ($arResult['TASKS']['RUNNING_CNT'] > 1) echo 'steps-arrow-right-right'?>"></span>
		<span class="bp-short-process-step-wrapper <?php if ($task['USERS_CNT'] > 1) echo 'bp-short-process-step-wrapper-more'?>">
			<a href="<?=SITE_DIR.'mobile/users/?user_id='.(int)$face['USER_ID']?>" class="bp-short-process-step  <?php if ($face['STATUS'] == CBPTaskUserStatus::Ok || $face['STATUS'] == CBPTaskUserStatus::Yes) echo 'bp-short-process-step-ready'?>
			<?php if ($face['STATUS'] == CBPTaskUserStatus::No || $face['STATUS'] == 4) echo 'bp-short-process-step-cancel'?> <?php if ($task['USERS_CNT'] > 1) echo 'bp-short-process-step-more'?>">
