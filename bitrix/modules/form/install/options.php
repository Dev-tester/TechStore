...
<?php 
$tabControl->Begin();
?><form method="POST" action="<?php echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialchars($mid)?>&lang=<?=LANGUAGE_ID?>"><?=bitrix_sessid_post()?><?php 
...
?>
	...
	?>
	<tr>
		<td valign="top"><?php 	if($type[0]=="checkbox")
		...
		<td valign="top" nowrap><?php 
			if($type[0]=="checkbox"):
				?><input type="checkbox" name="<?php echo htmlspecialchars($Option[0])?>" id="<?php echo htmlspecialchars($Option[0])?>" value="Y"<?php if($val=="Y")echo" checked";?>><?php 
			elseif($type[0]=="text"):
				?><input type="text" size="<?php echo $type[1]?>" maxlength="255" value="<?php echo htmlspecialchars($val)?>" name="<?php echo htmlspecialchars($Option[0])?>"><?php 
			elseif($type[0]=="textarea"):
				?><textarea rows="<?php echo $type[1]?>" cols="<?php echo $type[2]?>" name="<?php echo htmlspecialchars($Option[0])?>"><?php echo htmlspecialchars($val)?></textarea><?php 
			endif;
			?></td>
	</tr>
	<?php 
...
function RestoreDefaults()
{
	if(confirm('<?php echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>'))
		window.location = "<?php echo $APPLICATION->GetCurPage()?>?RestoreDefaults=Y&lang=<?=LANGUAGE_ID?>&mid=<?php echo urlencode($mid)?>";
}
</script>
