...
<input type="hidden" name="ID" value="<?php  echo $couponID; ?>">
<?php 
if ($subWindow)
{
	?><input type="hidden" name="DISCOUNT_ID" value="<?php  echo $discountID; ?>">
	<input type="hidden" name="MULTI" value="<?php  echo ($multiCoupons ? 'Y' : 'N');?>"><?php 
}
if ($copy)
{
...
else
{
	...
	elseif (!$subWindow)
	{
		...
		else
		{
			...
			<td width="40%"><?php  echo $control->GetCustomLabelHTML(); ?></td>
			<td width="60%">
				<?php  echo Loc::getMessage('BX_SALE_DISCOUNT_COUPON_MESS_DISCOUNT_ABSENT'); ?> <a href="<?php  echo $discountEditPath ?>"><?php 
	...
	?><tr id="tr_COUPON" class="adm-detail-required-field">
		...
		<td width="60%" id="td_COUPON_VALUE">
			<input type="text" id="COUPON" name="COUPON" size="32" maxlength="32" value="<?php  echo htmlspecialcharsbx($coupon['COUPON']); ?>" />&nbsp;
			<input type="button" value="<?php  echo Loc::getMessage('BX_SALE_DISCOUNT_COUPON_FIELD_COUPON_GENERATE'); ?>" id="COUPON_GENERATE">
		</td>
	</tr><?php 
