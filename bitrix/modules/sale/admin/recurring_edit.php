...
<form method="POST" action="<?php echo $APPLICATION->GetCurPage()?>?" name="frecurring_edit">
...
$tabControl->BeginNextTab();
	if ($ID > 0):?>
		...
		<tr>
			<td><?php echo GetMessage("SRE_TIMESTAMP")?></td>
			<td><?=$str_TIMESTAMP_X?></td>
		</tr>
	<?php endif;?>
	<tr class="adm-detail-required-field">
		<td width="40%"><?php echo GetMessage("SRE_USER1")?></td>
		<td width="60%"><?php 
			$user_name = "";
	...
	<tr>
		<td><?php echo GetMessage("SRE_CANCELED")?></td>
		<td>
			<input type="checkbox" name="CANCELED" value="Y"<?php if ($str_CANCELED=="Y") echo " checked"?>>
		</td>
	</tr>
	<tr>
		<td valign="top"><?php echo GetMessage("SRE_CANCEL_REASON")?></td>
		<td valign="top">
			<textarea name="CANCELED_REASON" rows="2" cols="40"><?= $str_CANCELED_REASON ?></textarea>
		</td>
	</tr>
	<tr>
		<td><?php echo GetMessage("SRE_MODULE")?></td>
		<td>
			<?php if ($saleModulePermissions >= "W"):?>
				...
				<?php else:?>
					<select name="MODULE" OnChange="ModuleChange()">
						...
						{
							?><option value="<?= $arModuleList["ID"] ?>"<?php if ($str_MODULE == $arModuleList["ID"]) echo " selected";?>><?= htmlspecialcharsEx($arModuleList["ID"]) ?></option><?php 
						}
						?>
					</select>
				<?php endif;?>
	...
	<?php if ($saleModulePermissions >= "W"):?>
		<tr>
			<td><?php echo GetMessage("SRE_PRODUCT_NAME")?></td>
			<td>
				<input type="text" name="PRODUCT_NAME" size="30" maxlength="250" value="<?= $str_PRODUCT_NAME; ?>">
			</td>
		</tr>
		<tr>
			<td><?php echo GetMessage("SRE_PRODUCT_URL")?></td>
			<td>
	...
	<?php endif;?>
	<tr>
		<td valign="top"><?php echo GetMessage("SRE_LAST_DATE")?>:</td>
		<td valign="top">
			<?= CalendarDate("PRIOR_DATE", $str_PRIOR_DATE, "frecurring_edit", "20", "class=\"typeinput\""); ?>
		</td>
	</tr>
	<tr class="adm-detail-required-field">
		<td valign="top"><?php echo GetMessage("SRE_NEXT_DATE")?>:</td>
		<td valign="top">
			<?= CalendarDate("NEXT_DATE", $str_NEXT_DATE, "frecurring_edit", "20", "class=\"typeinput\""); ?>
		</td>
	</tr>
	<tr>
		<td><?php echo GetMessage("SRE_LAST_SUCCESS")?></td>
		<td>
			<input type="checkbox" name="SUCCESS_PAYMENT" value="Y"<?php if ($str_SUCCESS_PAYMENT=="Y") echo " checked"?>>
		</td>
	</tr>
	<tr>
		<td><?php echo GetMessage("SRE_STEPS")?></td>
		<td>
			<input type="text" name="REMAINING_ATTEMPTS" size="5" maxlength="5" value="<?= $str_REMAINING_ATTEMPTS ?>">
		</td>
	</tr>
	<tr class="adm-detail-required-field">
		<td><?php echo GetMessage("SRE_BASE_ORDER")?></td>
		<td>
			<input type="text" name="ORDER_ID" size="10" maxlength="10" value="<?= $str_ORDER_ID ?>">
		</td>
	</tr>
	<tr>
		<td valign="top"><?php echo GetMessage("SRE_DESCRIPTION")?></td>
		<td valign="top">
