...
if ($arParams["AJAX_CALL"] != "Y"
	...
	<?php endforeach;?>
	<div><input <?php  if($isChecked!="Y") echo 'checked';?> type="radio" onclick="clearLocInput();" name="NEW_LOCATION_<?=$arParams["ORDER_PROPS_ID"]?>" value="0" id="loc_0" /><label for="loc_0"><?=GetMessage("LOC_DEFAULT_NAME_NULL")?>:</label></div>
<?php endif;?>

<input
	size="<?=$arParams["SIZE1"]?>"
	name="<?php echo $arParams["CITY_INPUT_NAME"]?>_val"
	id="<?php echo $arParams["CITY_INPUT_NAME"]?>_val"
	value="<?=$arResult["LOCATION_STRING"]?>"
	class="search-suggest" type="text"
	autocomplete="off"
	onfocus="loc_sug_CheckThis(this, this.id);"
	<?=($arResult["SINGLE_CITY"] == "Y" ? " disabled" : "")?>/>
<input type="hidden" name="<?php echo $arParams["CITY_INPUT_NAME"]?>" id="<?php echo $arParams["CITY_INPUT_NAME"]?>" value="<?=$arParams["LOCATION_VALUE"]?>">
<script type="text/javascript">
	...
	document.loc_sug_CheckThis = function(oObj, id)
	{
		try
		{
			...
			{
				window.oObject[oObj.id] = new JsSuggestSale(oObj, '<?php echo $arResult["ADDITIONAL_VALUES"]?>', '', '', '<?=CUtil::JSEscape($arParams["ONCITYCHANGE"])?>');
				return;
			}
			else
			{
	...
	{				
		var inp = BX("<?php echo $arParams["CITY_INPUT_NAME"]?>_val");			
		if(inp)
		{
			inp.value = "";
			inp.focus();
