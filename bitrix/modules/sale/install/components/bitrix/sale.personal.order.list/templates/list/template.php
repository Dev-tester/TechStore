...
<table border="0" cellspacing="0" cellpadding="5">
	<tr>
		<td width="60%">
			...
			{
				?><a href="<?=$arResult["CURRENT_PAGE"]?>?filter_history=N"><?php echo GetMessage("STPOL_CUR_ORDERS")?></a><?php 
			}
			else
			{
				?><a href="<?=$arResult["CURRENT_PAGE"]?>?filter_history=Y&filter_status=F"><?php echo GetMessage("STPOL_ORDERS_HISTORY")?></a><?php 
			}
			echo "<br /><br />";
			$bNoOrder = true;
			foreach($arResult["ORDER_BY_STATUS"] as $key => $val)
			{
				...
				foreach($val as $vval)
				{
					...
					if($bShowStatus)
					{
						?><h2><?php echo GetMessage("STPOL_STATUS")?> "<?=$arResult["INFO"]["STATUS"][$key]["NAME"] ?>"</h2>
					...
					<table class="sale_personal_order_list">
						<tr>
							<td>
								<b>
								<?php echo GetMessage("STPOL_ORDER_NO")?>
								<a title="<?php echo GetMessage("STPOL_DETAIL_ALT")?>" href="<?=$vval["ORDER"]["URL_TO_DETAIL"] ?>"><?=$vval["ORDER"]["ACCOUNT_NUMBER"]?></a>
								<?php echo GetMessage("STPOL_FROM")?>
							...
							<td>
								<table class="sale_personal_order_list_table">
									<tr>
										<td width="0%">&nbsp;&nbsp;&nbsp;&nbsp;</td>
										<td width="100%">
											<b><?php echo GetMessage("STPOL_CONTENT")?></b>
										</td>
										<td width="0%">&nbsp;</td>
									</tr>
									<?php 
						...
						<tr>
							<td align="right">
								...
								<?php if ($vval["ORDER"]["CAN_CANCEL"] == "Y"):?>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			...
			if ($bNoOrder)
			{
				?><center><?php echo GetMessage("STPOL_NO_ORDERS")?></center><?php 
			}
			?>
		</td>
		<td width="5%" rowspan="3">&nbsp;</td>
		<td width="35%" rowspan="3" valign="top">
			<?php echo GetMessage("STPOL_HINT")?><br /><br />
			<?php echo GetMessage("STPOL_HINT1")?>
		</td>
	</tr>
</table>