...
<table class="header">
	<tr>
		<?php  if ($params["BILLBR_PATH_TO_LOGO"]) { ?>
		<td style="padding-right: 5pt; ">
			<?php  $imgParams = CFile::_GetImgParams($params['BILLBR_PATH_TO_LOGO']);
...
<table width="100%">
	<tr>
		...
		<?php  } ?>
		<td align="right">
			<?php if ($params['BILLBR_PAYER_SHOW'] === 'Y'):?>
				<table class="inv">
					<tr align="right">
...
<?php if ($params['BILLBR_PAYER_SHOW'] === 'Y' || $params["BUYER_PERSON_COMPANY_NAME"]):?>
	<br>
	<br>
	<br>
<?php endif;?>
...
<?php  if ($params["BILLBR_COMMENT1"] || $params["BILLBR_COMMENT2"]) { ?>
<b><?=Loc::getMessage('SALE_HPS_BILLBR_COMMENT')?></b>
<br>
	<?php  if ($params["BILLBR_COMMENT1"]) { ?>
	...
	<tr>
		...
		<?php  if ($bankAccNo && $bankRouteNo && $bankSwift) { ?>
			...
			<?php  if ($bank || $bankAddr || $bankPhone) { ?>
				<?=Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_2')?>: <?php  if ($bank) { ?><?=$bank; ?><?php  } ?>
				<br>
