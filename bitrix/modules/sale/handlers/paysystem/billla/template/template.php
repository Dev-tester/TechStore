...
<table class="header">
	<tr>
		<?php  if ($params["BILLLA_PATH_TO_LOGO"]) { ?>
		<td style="padding-right: 5pt; ">
			<?php  $imgParams = CFile::_GetImgParams($params['BILLLA_PATH_TO_LOGO']);
...
<table width="100%">
	<tr>
		...
		<?php  } ?>
		<td align="right">
			<?php if ($params['BILLLA_PAYER_SHOW'] === 'Y'):?>
				<table class="inv">
					<tr align="right">
...
<?php if ($params['BILLLA_PAYER_SHOW'] === 'Y' || $params["BUYER_PERSON_COMPANY_NAME"]):?>
	<br>
	<br>
	<br>
<?php endif;?>
...
<?php  if ($params["BILLLA_COMMENT1"] || $params["BILLLA_COMMENT2"]) { ?>
<b><?=Loc::getMessage('SALE_HPS_BILLLA_COMMENT')?></b>
<br>
	<?php  if ($params["BILLLA_COMMENT1"]) { ?>
	...
	<tr>
		...
		<?php  if ($bankAccNo && $bankRouteNo && $bankSwift) { ?>
			...
			<?php  if ($bank || $bankAddr || $bankPhone) { ?>
				<?=Loc::getMessage('SALE_HPS_BILLLA_COMPANY_BANK_2')?>: <?php  if ($bank) { ?><?=htmlspecialcharsbx($bank); ?><?php  } ?>
				<br>
