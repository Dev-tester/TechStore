...
<table class="header">
	<tr>
		<?php  if ($params["BILLEN_PATH_TO_LOGO"]) { ?>
		<td style="padding-right: 5pt; ">
			<?php  $imgParams = CFile::_GetImgParams($params['BILLEN_PATH_TO_LOGO']);
		...
		<td>
			...
			if ($params["SELLER_COMPANY_ADDRESS"]) {
				...
				{
					?><b><?= nl2br(htmlspecialcharsbx($sellerAddress)) ?></b><?php 
				}
				unset($sellerAddress);
				?><br><?php 
			} ?>
...
<?php if ($params['BILLEN_PAYER_SHOW'] == 'Y'):?>
	<table width="100%">
		<tr>
			<?php  if ($params["BUYER_PERSON_COMPANY_NAME"]) { ?>
			<td>
				<b>To</b><br>
				<?=htmlspecialcharsbx($params["BUYER_PERSON_COMPANY_NAME"]); ?><br><?php 
				if ($params["BUYER_PERSON_COMPANY_ADDRESS"]) {
					...
					if (is_array($buyerAddress))
					{
						if (!empty($buyerAddress))
						{
							...
							?><div style="display: inline-block; vertical-align: top;"><?= $addrValue ?><br></div><?php 
							unset($addrValue);
						}
					}
					else
					{
						?><?= nl2br(htmlspecialcharsbx($buyerAddress)) ?><br><?php 
					}
					unset($buyerAddress);
				}
...
?>
	<tr valign="top">
		<?php foreach ($arCols as $columnId => $cols):?>
			<?php 
...
<?php  if ($params["BILLEN_COMMENT1"] || $params["BILLEN_COMMENT2"]) { ?>
<b>Terms & Conditions</b>
<br>
	<?php  if ($params["BILLEN_COMMENT1"]) { ?>
	...
	<tr>
		...
		<?php  if ($bankAccNo && $bankRouteNo && $bankSwift) { ?>
			...
			<?php  if ($bank || $bankAddr || $bankPhone) { ?>
				Bank Name and Address: <?php  if ($bank) { ?><?=htmlspecialcharsbx($bank); ?><?php  } ?>
				<br>
