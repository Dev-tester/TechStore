...
<?php if (CSalePaySystemAction::GetParamValue('QUOTE_BR_HEADER_SHOW') == 'Y'):?>
<table class="header">
	<tr><?php 
		...
		if ($pathToLogo)
		{
			...
			?><td style="padding-right: 5pt; "><img src="<?=$imgParams['SRC']; ?>" width="<?=$imgWidth; ?>" /></td><?php 
		}
		?>
		<td><?php 
			...
			<?php  } ?>
			<?php  if (CSalePaySystemAction::GetParamValue("SELLER_EMAIL", false)) { ?>
...
<div style="margin: 0pt; padding: 0pt;">
	<?php  if (CSalePaySystemAction::GetParamValue("BUYER_NAME", false)) { ?>
		...
		if ($buyerPayerName)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_BR_TEXT_CONTACT_PERSON', null, $lng).": %s", $buyerPayerName); ?><br><?php 
		}
		unset($buyerPayerName);
		$buyerPhone = CSalePaySystemAction::GetParamValue("BUYER_PHONE", false);
		if ($buyerPhone)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_BR_TEXT_PHONE', null, $lng).": %s", $buyerPhone); ?><br><?php 
		}
		unset($buyerPhone);
		$buyerFax = CSalePaySystemAction::GetParamValue("BUYER_FAX", false);
		if ($buyerFax)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_BR_TEXT_FAX', null, $lng).": %s", $buyerFax); ?><br><?php 
		}
		unset($buyerFax);
		$buyerEmail = CSalePaySystemAction::GetParamValue("BUYER_EMAIL", false);
		if ($buyerEmail)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_BR_TEXT_EMAIL', null, $lng).": %s", $buyerEmail); ?><br><?php 
		}
		unset($buyerEmail);
		?>
	<?php  } ?>
...
<?php if (CSalePaySystemAction::GetParamValue('QUOTE_BR_SIGN_SHOW') == 'Y'):?>
	<?php  if (!$blank) { ?>
