...
<div class="crm-automation-popup-settings">
	...
	<?php if(empty($template['Options']))
	{
		?><b style="color: #ff5752;"><?=GetMessage('CRM_GEDA_NO_TEMPLATES');?></b><?php 
	}
	else
	{?>
	<select class="crm-automation-popup-settings-dropdown" name="<?=htmlspecialcharsbx($template['FieldName'])?>" id="id_template_id">
		<?php foreach ($template['Options'] as $value => $optionLabel):?>
...
<div class="crm-automation-popup-settings">
	<label class="crm-automation-popup-chk-label">
		<input type="checkbox" name="<?=$map['UseSubscription']['FieldName'];?>" value="Y" class="crm-automation-popup-chk"<?php echo ($dialog->getCurrentValue($map['UseSubscription']['FieldName']) != 'Y' ? '' : ' checked');?>>
