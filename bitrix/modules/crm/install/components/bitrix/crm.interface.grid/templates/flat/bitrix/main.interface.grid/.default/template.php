...
<table cellspacing="0" class="bx-crm-interface-grid<?php if($arResult["OPTIONS"]["theme"] <> '') echo " bx-crm-interface-grid-theme-".$arResult["OPTIONS"]["theme"]?>" id="<?=$arParams["GRID_ID"]?>">
	<thead>
...
<?php endforeach?>
	...
	<tr class="bx-crm bx-crm-grid-head" oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.settingsMenu"<?php if($GLOBALS['USER']->IsAuthorized()):?> ondblclick="bxGrid_<?=$arParams["GRID_ID"]?>.EditCurrentView()"<?php endif?>>
<?php if($arResult["ALLOW_EDIT"]):?>
		<td class="bx-crm-checkbox-col" width="1%"><input type="checkbox" name="" id="<?=$arParams["GRID_ID"]?>_check_all" value="" title="<?php echo GetMessage("interface_grid_check_all")?>" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.SelectAllRows(this);"></td>
<?php endif?>
		<td class="bx-crm-actions-col" width="1%"><a draggable="false" href="javascript:void(0);"
			...
			title="<?php echo GetMessage("interface_grid_settings")?>" class="bx-crm-action"><div class="empty"></div></a></td>
<?php 
...
?>
	<tr class="bx-crm-table-body" oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.oActions[<?=$index?>]"<?php if($sDefAction <> ''):?> ondblclick="<?=htmlspecialcharsbx($sDefAction)?>"<?php endif?>>
<?php if($arResult["ALLOW_EDIT"]):?>
	<?php 
	if($aRow["editable"] !== false):
		$data_id = ($aRow["id"] <> ''? $aRow["id"] : $aRow["data"]["ID"]);
	?>
		<td class="bx-crm-checkbox-col"><input type="checkbox" name="ID[]" id="ID_<?=$data_id?>" value="<?=$data_id?>" title="<?php echo GetMessage("interface_grid_check")?>"></td>
	<?php else:?>
...
<?php endif?>
	<?php if(is_array($aRow["actions"]) && count($aRow["actions"]) > 0):?>
		<td class="bx-crm-actions-col"><a draggable="false" href="javascript:void(0);"
			...
			title="<?php echo GetMessage("interface_grid_act")?>" class="bx-crm-action"><div class="empty"></div></a></td>
	<?php else:?>
		<td>&nbsp;</td>
	<?php endif?>
<?php 
...
?>
	<tr><td class="bx-crm-not-result" colspan="<?=$colspan?>"><?php echo GetMessage("interface_grid_no_data")?></td></tr>
<?php endif?>
	</tbody>
...
<!--		<td><div class="bx-crm-separator"></div></td>-->
	<?php endif;?>
		<td class="vam"><a href="javascript:void(0);" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ActionEdit(this);" title="<?php echo GetMessage("interface_grid_edit_selected")?>" class="bx-crm-bd-icon-edit" id="edit_button_<?=$arParams["GRID_ID"]?>"></a></td>
<?php 
	$bNeedSep = true;
endif;
?>
...
<?php endif?>
<?php if($bShowApply):?>
		<td class="vam" style="padding-left:2px;"><input class="bx-crm-btn bx-crm-btn-medium bx-crm-btn-gray mb0" type="submit" name="apply" value="<?php echo GetMessage("interface_grid_apply")?>" disabled></td>
<?php endif?>
<!--				</tr>-->
<!--			</table>-->
<!--		</td>-->
