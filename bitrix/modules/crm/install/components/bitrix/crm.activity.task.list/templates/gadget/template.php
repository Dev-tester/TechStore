...
else
{
	foreach($arResult['TASK'] as $arTask)
	{
		...
		<div class="crm-task-element">
			...
			<div class="crm-task-element-title"><span><?=GetMessage('CRM_ENTITY_'.$arTask['ENTITY_TYPE'])?></span> <a href="<?=$arTask['ENTITY_LINK']?>" bx-tooltip-user-id="<?=$arTask['ENTITY_TYPE']?>_<?=$arTask['ENTITY_ID']?>" bx-tooltip-loader="<?=htmlspecialcharsbx('/bitrix/components/bitrix/crm.'.strtolower($arTask['ENTITY_TYPE']).'.show/card.ajax.php')?>" bx-tooltip-classname="crm_balloon<?=($arTask['ENTITY_TYPE'] == 'LEAD' || $arTask['ENTITY_TYPE'] == 'DEAL' || $arTask['ENTITY_TYPE'] == 'QUOTE' ? '_no_photo': '_'.strtolower($arTask['ENTITY_TYPE']))?>"><?=$arTask['ENTITY_TITLE']?></a></div>
			<?php endif;?>
