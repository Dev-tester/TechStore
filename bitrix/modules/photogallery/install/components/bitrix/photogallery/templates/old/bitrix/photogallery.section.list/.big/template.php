...
?><div class="photo-controls photo-action"><?php 
if (!empty($arResult["SECTION"]["BACK_LINK"])):
	?><noindex><a rel="nofollow" href="<?=$arResult["SECTION"]["BACK_LINK"]?>" title="<?=GetMessage("P_UP_TITLE")?>" class="photo-action back-to-album" <?php 
		?>><?=GetMessage("P_UP")?></a></noindex><?php 
endif;
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" class="photo-album" id="photo_album_info_<?=$res["ID"]?>">
						...
						<div class="inner">
							<a href="<?=$res["LINK"]?>">
								<div class="photo-album-cover" id="photo_album_cover_<?=$res["ID"]?>" <?php 
									?>style="width:<?=$arParams["ALBUM_PHOTO_SIZE"]?>px; height:<?=$arParams["ALBUM_PHOTO_SIZE"]?>px;<?php 
								if (!empty($res["DETAIL_PICTURE"]["SRC"])):
									?>background-image:url('<?=$res["DETAIL_PICTURE"]["SRC"]?>');<?php 
								endif;
	...
	<td>
		<div class="photo-album-info">
			<a href="<?=$res["LINK"]?>">
				<div class="password" id="photo_album_password_<?=$res["ID"]?>" title="<?=GetMessage("P_PASSWORD")?>" <?php 
...
<?php 
			...
			if (intVal($res["SECTIONS_CNT"]) > 0 && $arParams["PERMISSION"] >= "U"):
				?><div class="photo-album-cnt-album"><?=GetMessage("P_ALBUMS_CNT")?>: <a href="<?=$res["LINK"]?>"><?=$res["SECTIONS_CNT"]?></a></div><?php 
			endif;


			?><div class="photo-controls photo-album-controls"><?php 

			if (!empty($res["EDIT_LINK"])):
				?><noindex><a rel="nofollow" href="<?=$res["EDIT_LINK"]?>" class="photo-action album-edit" <?php 
					?>onclick="EditAlbum('<?=CUtil::JSEscape($res["EDIT_LINK"])?>'); return false;"><?php 
					?><?=GetMessage("P_SECTION_EDIT")?></a></noindex><?php 
			endif;

			if (!empty($res["EDIT_ICON_LINK"])):
				?><noindex><a rel="nofollow" href="<?=$res["EDIT_ICON_LINK"]?>" class="photo-action album-edit-icon" <?php 
					?>onclick="EditAlbum('<?=CUtil::JSEscape($res["EDIT_ICON_LINK"])?>'); return false;"><?php 
					?><?=GetMessage("P_EDIT_ICON")?></a></noindex><?php 
			endif;

			if (!empty($res["DROP_LINK"])):
				?><noindex><a rel="nofollow" href="<?=$res["DROP_LINK"]?>" class="photo-action album-delete" <?php 
					?>onclick="return confirm('<?=GetMessage('P_SECTION_DELETE_ASK')?>');" class="edit"><?php 
