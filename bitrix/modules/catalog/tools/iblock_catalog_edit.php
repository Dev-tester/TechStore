...
?>
	...
	<input type="hidden" name="IBLOCK_ID" value="<?php  echo $iblockId; ?>">
<?php 
...
?><tr id="tr_CATALOG">
	<td style="width: 40%;"><?php  echo $control->GetCustomLabelHTML(); ?></td>
	<td style="width: 60%;">
...
?><tr id="tr_SKU">
	<td style="width: 40%; vertical-align: top;"><?php  echo $control->GetCustomLabelHTML(); ?></td>
	<td style="width: 60%;"><?php 
		...
		if ($catalog['CATALOG_TYPE'] == CCatalogSKU::TYPE_OFFERS)
		{
			...
			?><input type="hidden" name="SKU" value="<?php  echo $productIblockId; ?>"><?php 
		}
		else
		{
			if ($catalog['CATALOG_TYPE'] == CCatalogSKU::TYPE_PRODUCT || $catalog['CATALOG_TYPE'] == CCatalogSKU::TYPE_FULL)
			...
			<input data-checkbox="Y" type="checkbox" name="USE_SKU" value="Y" id="USE_SKU_Y"<?php  echo ($showSku ? ' checked' : ''); ?>>
			<div id="sku_data" style="display: <?php  echo ($showSku ? 'block' : 'none'); ?>;">
			<?php 
			if (!empty($offerList))
			{
...
if ($showSubscription)
{
	...
	?>
	<tr id="tr_SUBSCRIPTION" style="display: <?php  echo $rowDisplay; ?>;">
	<td style="width: 40%;"><?php  echo $control->GetCustomLabelHTML(); ?></td>
	<td style="width: 60%;"><?php 
	if ($enableSaleRecurring)
	{
		...
		{
			$hiddenValue = 'N';
			?><br><?php  echo Loc::getMessage('BX_CAT_IBLOCK_CATALOG_MESS_SUBSCRIPTION_WITH_SKU');
		}
...
?>
<tr id="tr_YANDEX_EXPORT" style="display: <?php  echo $rowDisplay; ?>;">
	<td style="width: 40%;"><?php  echo $control->GetCustomLabelHTML(); ?></td>
	<td style="width: 60%;">
		<input type="hidden" id="YANDEX_EXPORT_N" name="YANDEX_EXPORT" value="N">
		<input type="checkbox" id="YANDEX_EXPORT_Y" name="YANDEX_EXPORT" value="Y"<?php  echo($catalog['YANDEX_EXPORT'] == 'Y' ? ' checked' : '');?>>
	</td>
</tr><?php 
...
?><tr id="tr_VAT_ID" style="display: <?php  echo $rowDisplay; ?>;">
	<td style="width: 40%;"><?php  echo $control->GetCustomLabelHTML(); ?></td>
	<td style="width: 60%;">
		<select name="VAT_ID">
		<?php 
		...
		{
			?><option value="<?php  echo $vatId; ?>"<?php  echo ($catalog['VAT_ID'] == $vatId ? ' selected' : ''); ?>><?php  echo htmlspecialcharsEx($vatName); ?></option><?php 
		}
		unset($vatId, $vatName);
		?>
		</select>
...
var iblockCatalogControl = new BX.Catalog.Admin.IblockCatalog(<?php  echo CUtil::PhpToJSObject($jsParams, false, false, true); ?>);
BX.ready(function()
{
