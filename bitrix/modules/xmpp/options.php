...
<?php 
$tabControl->Begin();
?><form method="POST" action="<?php echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialcharsbx($mid)?>&lang=<?=LANGUAGE_ID?>" id="FORMACTION"><?php 
...
?>
	...
	<tr>
		...
		<td width="50%">
			<?php $val = COption::GetOptionString("xmpp", "domain_name", "");?>
	...
	<tr>
		...
		<td width="50%">
			<?php $val = COption::GetOptionString("xmpp", "listen_domain", "0.0.0.0");?>
	...
	<tr>
		...
		<td width="50%">
			<?php $val = COption::GetOptionString("xmpp", "domain_lang", "en");?>
	...
	<tr>
		...
		<td width="50%">
			<?php $val = COption::GetOptionString("xmpp", "php_path", (StrToUpper(substr(PHP_OS, 0, 3)) === "WIN") ? "../apache/php.exe -c ../apache/php.ini" : "php -c /etc/php.ini");?>
	...
	<!--tr>
		...
		<td valign="middle" width="50%">
			<?php $val = COption::GetOptionString("xmpp", "server_path", "./bitrix/modules/xmpp/xmppd.php");?>
	...
	<tr>
		...
		<td width="50%">
			<?php $val = intval(COption::GetOptionString("xmpp", "log_level", "4"));?>
	...
	<tr>
		...
		<td width="50%">
			<?php $val = StrToUpper(COption::GetOptionString("xmpp", "start_ssl", "N"));?>
	...
	<tr>
		...
		<td width="50%">
			<?php $val = COption::GetOptionInt("xmpp", "iblock_presence");?>
			<select name="iblock_presence">
				...
				{
					?><option value="<?= $arIBlock['ID'] ?>"<?= (($arIBlock['ID'] == $val) ? " selected" : "")?>><?= ($arIBlock['CODE'] ? '['.$arIBlock['CODE'].'] ' : '').$arIBlock['NAME'] ?></option><?php 
				}
				?>
			</select>
	</tr>
	<tr>
		...
		<td width="50%">
			<?php $curVal = str_replace(array("#NOBR#","#/NOBR#"), array("",""), COption::GetOptionString("xmpp", "name_template", "#LAST_NAME# #NAME#"));?>
...
<?php $tabControl->BeginNextTab();?>
	...
	<tr>
		...
		<td width="50%">
			<?php $val = COption::GetOptionString("xmpp", "sonet_sender_type", "jid");?>
	...
	<tr>
		...
		<td width="50%">
			<?php $val = COption::GetOptionString("xmpp", "sonet_jid", "admin@".$_SERVER["SERVER_NAME"]);?>
			<input type="text" size="35" maxlength="255" value="<?=htmlspecialcharsbx($val)?>" name="sonet_jid" id="sonet_jid"></td>
	</tr>
	<tr>
		<td width="50%"><?= GetMessage("XMPP_OPT_SONET_UID") ?>:</td>
		<td width="50%">
			<?php $val = COption::GetOptionString("xmpp", "sonet_uid", "");?>
...
<input <?php if (!$USER->IsAdmin()) echo "disabled" ?> class="adm-btn-save" type="submit" name="Update" value="<?= GetMessage("XMPP_OPT_ACT_APPLY") ?>">
<input type="reset" name="reset" value="<?= GetMessage("XMPP_OPT_ACT_DEFAULT") ?>">
<?php $tabControl->End();?>
</form>

<?php echo BeginNote();?>
<span class="required"><sup>1</sup></span> <?php echo GetMessage("XMPP_OPT_NOTE_1")?><br>
<span class="required"><sup>2</sup></span> <?php echo GetMessage("XMPP_OPT_NOTE_2")?>
<?php echo EndNote();?>
<?php endif;?>
