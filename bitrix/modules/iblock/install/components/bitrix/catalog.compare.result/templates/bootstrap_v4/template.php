...
?>
<div class="catalog-compare mb-4 bx-<?php  echo $templateData['TEMPLATE_CLASS']; ?>" id="bx_catalog_compare_block"><?php 
	if ($isAjax)
	{
	...
	<div class="mb-3 d-flex align-items-center">
		...
		<div class="pr-2">
			<a
				class="btn btn-sm <?php  echo (!$arResult["DIFFERENT"] ? 'btn-secondary' : 'btn-primary'); ?>"
				href="<?php  echo $arResult['COMPARE_URL_TEMPLATE'].'DIFFERENT=N'; ?>"
				rel="nofollow"><?=GetMessage("CATALOG_ALL_CHARACTERISTICS")?></a>
		</div>
		<div class="pr-2">
			<a
				class="btn btn-sm <?php  echo ($arResult["DIFFERENT"] ? 'btn-secondary' : 'btn-primary'); ?>"
				href="<?php  echo $arResult['COMPARE_URL_TEMPLATE'].'DIFFERENT=Y'; ?>"
				rel="nofollow"><?=GetMessage("CATALOG_ONLY_DIFFERENT")?></a>
		</div>
	</div>
	<?php  if (!empty($arResult["ALL_FIELDS"]) || !empty($arResult["ALL_PROPERTIES"]) || !empty($arResult["ALL_OFFER_FIELDS"]) || !empty($arResult["ALL_OFFER_PROPERTIES"]))
	{
		?>
		<div class="catalog-compare-filter p-3 mb-3">
			<div class="catalog-compare-filter-title"><?=GetMessage("CATALOG_COMPARE_PARAMS")?></div>
			<div class="row">
				<?php  if (!empty($arResult["ALL_FIELDS"]))
				{
						...
						if (!isset($arResult['FIELDS_REQUIRED'][$propCode]))
						{
							...
							<div class="col-12 col-lg-3 col-md-4">
								<span class="form-check" onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arProp["ACTION_LINK"])?>')">
									<input type="checkbox" class="form-check-input" id="PF_<?=$propCode?>"<?php  echo ($arProp["IS_DELETED"] == "N" ? ' checked="checked"' : ''); ?>>
				...
				if (!empty($arResult["ALL_OFFER_FIELDS"]))
				{
					foreach($arResult["ALL_OFFER_FIELDS"] as $propCode => $arProp)
					{
						...
						<div class="col-12 col-lg-3 col-md-4">
							<span class="form-check" onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arProp["ACTION_LINK"])?>')">
								<input type="checkbox" class="form-check-input" id="OF_<?=$propCode?>"<?php  echo ($arProp["IS_DELETED"] == "N" ? ' checked="checked"' : ''); ?>>
				...
				if (!empty($arResult["ALL_PROPERTIES"]))
				{
					foreach($arResult["ALL_PROPERTIES"] as $propCode => $arProp)
					{
						...
						<div class="col-12 col-lg-3 col-md-4">
							<span class="form-check" onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arProp["ACTION_LINK"])?>')">
								<input type="checkbox" class="form-check-input" id="PP_<?=$propCode?>"<?php echo ($arProp["IS_DELETED"] == "N" ? ' checked="checked"' : '');?>>
				...
				if (!empty($arResult["ALL_OFFER_PROPERTIES"]))
				{
					foreach($arResult["ALL_OFFER_PROPERTIES"] as $propCode => $arProp)
					{
						...
						<div class="col-12 col-lg-3 col-md-4">
							<span class="form-check" onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arProp["ACTION_LINK"])?>')">
								<input type="checkbox" class="form-check-input" id="OP_<?=$propCode?>"<?php  echo ($arProp["IS_DELETED"] == "N" ? ' checked="checked"' : ''); ?>>
		...
		<table class="table">
			...
			<tr>
				...
				<?php  foreach ($arResult["ITEMS"] as $arElement)
				{
					...
					{
						?>
						<td><?php  echo $arElement['MIN_PRICE']['PRINT_DISCOUNT_VALUE']; ?></td>
						<?php 
					}
			...
			<?php  if (!empty($arResult["SHOW_PROPERTIES"]))
			{
