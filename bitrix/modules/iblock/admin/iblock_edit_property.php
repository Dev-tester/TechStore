...
<tr>
	...
	<td>
		...
		<select  onchange="if(this.selectedIndex!==0) document.getElementById('CURRENT_PROPERTY_FILE_TYPE').value=this[this.selectedIndex].value">
			...
			<option value="jpg, gif, bmp, png, jpeg"<?php if("jpg, gif, bmp, png, jpeg" == $arProperty['FILE_TYPE'])echo " selected"?>><?php echo GetMessage("BT_ADM_IEP_PROP_FILE_TYPES_PIC")?></option>
...
function add_list_row()
{
	...
	oCell = newRow.insertCell(-1);
	strContent = '<?php  echo CUtil::JSEscape(__AddListValueDefCell($defaultListValueSettings['ID'], $defaultListValueSettings)); ?>';
	strContent = strContent.replace(/tmp_xxx/ig, id);
	oCell.innerHTML = strContent;
