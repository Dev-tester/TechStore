...
?>
	...
	<?php  if (strlen($zr["DATE_LOCK"])>0) : ?>
	<tr>
		<td><?=GetMessage("FLOW_DOCUMENT_DATE_LOCK")?></td>
		<td><?=$zr["DATE_LOCK"]?>&nbsp;&nbsp;[<a href="user_edit.php?ID=<?=$zr["LOCKED_BY"]?>&lang=<?=LANG?>" title="<?=GetMessage('FLOW_USER_ALT')?>"><?=$zr["LOCKED_BY"]?></a>]&nbsp;<?php  echo htmlspecialcharsbx($zr["LUSER_NAME"])?>&nbsp;<?php if ($zr["LOCKED_BY"]==$USER->GetID()):?><span class="required">(!)</span><?php endif;?></td>
	</tr>
	<?php  endif; ?>
	<tr>
