...
foreach ($arResult["TOPIC"] as $res)
{
	...
	<div class="frm-mp-info">
		<div class="frm-mp-info-inner">
			<div class="frm-mp-date intranet-date"><?php echo $res["LAST_POST_DATE"];?></div>
			<div class="frm-mp-post"><a href="<?=$res["read"]?>"><?php echo $res["TITLE"]?></a></div>
			...
			{
				?><div class="frm-mp-post"><?=GetMessage("FRM_MP_VIEWS")?> <?=$res["VIEWS"]?></div><?php 
			}
			if(intval($res["POSTS"]) > 0)
			{
				?><div class="frm-mp-post"><?=GetMessage("FRM_MP_POSTS")?> <?=$res["POSTS"]?></div><?php 
			}
			?>
