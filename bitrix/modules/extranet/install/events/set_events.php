...
while($lang = $langs->Fetch())
{
	...
	if(count($arSites) > 0)
	{
		...
		$emess->Add(array(
			...
			"MESSAGE" => "<?php EventMessageThemeCompiler::includeComponent(\"bitrix:intranet.template.mail\", \"\", array(\"USER_ID\" => \"{#USER_ID#}\",\"CHECKWORD\" => \"{#CHECKWORD#}\",\"SERVER_NAME\" => \"{#SERVER_NAME#}\",\"USER_TEXT\" => \"{#USER_TEXT#}\",\"USER_ID_FROM\" => \"{#USER_ID_FROM#}\",\"TEMPLATE_TYPE\" => \"EXTRANET_INVITATION\",\"FIELDS\" => \$arParams));?>",
			"BODY_TYPE" => "html",
		));		
	}
}
