...
<div class="bx_mfeedback bx_<?=$arResult["THEME"]?>">
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST">
		<?=bitrix_sessid_post()?>
		<strong><?=GetMessage("MFT_NAME")?><?php if(empty($arParams["REQUIRED_FIELDS"]) || in_array("NAME", $arParams["REQUIRED_FIELDS"])):?><span class="mf-req">*</span><?php endif?></strong></br>
		<input type="text" name="user_name" value="<?=$arResult["AUTHOR_NAME"]?>"/><br/>

		<strong><?=GetMessage("MFT_EMAIL")?><?php if(empty($arParams["REQUIRED_FIELDS"]) || in_array("EMAIL", $arParams["REQUIRED_FIELDS"])):?><span class="mf-req">*</span><?php endif?></strong></br>
		<input type="text" name="user_email" value="<?=$arResult["AUTHOR_EMAIL"]?>"/><br/>

		<strong><?=GetMessage("MFT_MESSAGE")?><?php if(empty($arParams["REQUIRED_FIELDS"]) || in_array("MESSAGE", $arParams["REQUIRED_FIELDS"])):?><span class="mf-req">*</span><?php endif?></strong></br>
