...
<div class="menu-items" id="menu-items">
	...
	<div class="menu-section">
		<div id="mb_user_profile_link" class="menu-item  menu-item-user"
			...
			<div class="menu-item-avatar"
				<?php  if ($arResult["USER"]["AVATAR"]): ?>style="background: url('<?= $arResult["USER"]["AVATAR"]["src"] ?>') no-repeat; background-size: cover;"<?php  endif ?>></div>
	...
	<div class="menu-section">
		...
		<div class="menu-item menu-item-wrap menu-icon-files" id="doc_user"
			onclick="webdavList('user/<?php  echo $GLOBALS['USER']->GetID(); ?>/');"><?= GetMessage("MB_CURRENT_USER_FILES_MAIN_MENU_ITEM_NEW"); ?></div>
	...
	<?php if (IsModuleInstalled('crm') && CModule::IncludeModule('crm') && CCrmPerms::IsAccessEnabled()):
	...
	if (is_array($arResult["GROUP_MENU"]) && count($arResult["GROUP_MENU"]) > 0):
		...
		foreach ($arResult["GROUP_MENU"] as $key => $value):
			?>
			<div class="menu-item menu-item-wrap" data-url="<?= $value[1] ?>"><?= $value[0] ?></div><?php 
		endforeach;
		?></div><?php 
	endif;
