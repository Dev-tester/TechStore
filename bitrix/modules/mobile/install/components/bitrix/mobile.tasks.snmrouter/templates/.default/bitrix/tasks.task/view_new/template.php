...
function getPriorityField($task, $can)
{
	...
	if($can['EDIT'])
	{
		$data = array_merge($data, [
			...
			'items' => [CTasks::PRIORITY_HIGH => GetMessage('MB_TASKS_BASE_SETTINGS_PRIORITY_VALUE')],
			'value' => [$task['PRIORITY']]
		]);
	}
	else
...
?><div id="post_inform_wrap_two" class=""><?php 
...
BX.ready(function(){
	...
	if(tabId === 'commentTab')
		...
		<?php
			if($task["~STATUS"] == CTasks::METASTATE_VIRGIN_NEW)
			{
				?>tabId = 'taskTab';<?php 
			}
			else if($task['COMMENTS_COUNT']>0)
			{
				?>tabId = 'commentTab';<?php 
			}
			else
			{
				?>tabId = 'taskTab';<?php 
