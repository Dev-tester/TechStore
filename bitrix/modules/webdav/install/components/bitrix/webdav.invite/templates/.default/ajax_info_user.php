...
{
	$ownerTitle = GetMessage('WD_INVITE_MODAL_OWNER_GROUP');
}
elseif(!empty($arResult['OWNER']['IS_SHARED']))
{
	$ownerTitle = GetMessage('WD_INVITE_MODAL_OWNER_USER_COMMON_SECTION');
}
else
{
	$ownerTitle = GetMessage('WD_INVITE_MODAL_OWNER_USER');
}
?>
...
<div class="bx-webdav-invite-access bx-webdav-invite-access-wide">
	<div class="bx-webdav-invite-access-title"><?= GetMessage('WD_INVITE_MODAL_TITLE_USAGE_SHARING'); ?> "<?= $arResult['TARGET_NAME'] ?>"</div>
	...
	<div class="bx-webdav-invite-footer">
		<a href="javascript:void(0)" onclick="wdOpenConfirmUnlink();" class="webform-button"><span class="webform-button-left"></span><span class="webform-button-text"><?= GetMessage('WD_INVITE_MODAL_BTN_DIE_SELF_ACCESS'); ?></span><span class="webform-button-right"></span></a>
	</div>
</div>
	...
	var createDialog = BX.create('div', {
		...
		children: [
			BX.create('div', {
				...
				text: "<?= GetMessageJS('WD_INVITE_MODAL_TITLE_DIE_SELF_ACCESS_SIMPLE');  ?>",
				children: []
			}),
			BX.create('div', {
				props: {
				...
				children: [
					...
					BX.create('span', {
						...
						text: "<?= GetMessageJS('WD_UNSHARE_SECTION_CONFIRM');  ?>"
					})
				]
			})
		]
		...
		buttons: [
			new BX.PopupWindowButton({
				...
				events : { click : function(e){
						...
						if(window.wdGlobalSectionDropUrl)
						{
							jsUtils.Redirect([], wdGlobalSectionDropUrl)
						}
						if(window.wdGlobalSectionDisconnectUrl)
						{
var waiterBox = BX.create('div', {
	...
	children: [
		...
		BX.create('span', {
			...
			text: '<?= GetMessageJS('WD_INVITE_MODAL_TAB_COMMON_SHARED_SECTION_PROCESS_DIE_ACCESS');  ?>'
		}),
		BX.create('a', {
			props: {
...
popupConfirm.show();
							...
							BX.ajax({
								'method': 'POST',
								'dataType': 'json',
								'url': window.wdGlobalSectionDisconnectUrl,
								'data': {
								...
								'onsuccess': function (data) {
									...
									BX.onCustomEvent('OnDisconnectSharedSection', [data]);
									...
									var messageBox = BX.create('div', {
										...
										children: [
											...
											BX.create('span', {
												...
												text: '<?= GetMessageJS('WD_INVITE_MODAL_TAB_COMMON_SHARED_SECTION_PROCESS_DIE_ACCESS_SUCCESS');  ?>'
											}),
											BX.create('div', {
												props: {
...
BX( function() {
	if (window.location.href.match(/[#]disconnect/)) {
		var section = window.location.href.match(/[#=]((sec)([0-9]+))/);
		var btnToShate = BX(section[1] + '-share');
		if (!wdGlobalRunDisconnectDialog && btnToShate) {
			wdOpenConfirmUnlink();
			wdGlobalRunDisconnectDialog = true;
		}
	}
});
