...
final class LivefeedIndexComment extends Stepper
{
	...
	public function execute(array &$result)
	{
		if (!(
			...
			&& Option::get('intranet', 'needLivefeedIndexComment', 'Y') == 'Y'
		))
		{
			return false;
		}
		...
		$params = Option::get("intranet", "livefeedindexcomment", "");
		...
		if ($params["count"] > 0)
		{
			$result["title"] = Loc::getMessage("FUPD_LF_INTRANET_COMMENT_INDEX_TITLE");
			$result["progress"] = 1;
			$result["steps"] = "";
			...
			if ($found)
			{
				Option::set("intranet", "livefeedindexcomment", serialize($params));
				$return = true;
			}
			...
			if ($found === false)
			{
				Option::delete("intranet", array("name" => "livefeedindexcomment"));
				Option::set('intranet', 'needLivefeedIndexComment', 'N');
			}
		}
		return $return;
	}
