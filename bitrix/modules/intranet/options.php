...
var arIB = <?php echo CUtil::PhpToJsObject($arIB)?>;
...
function change_iblock_list(value, index)
{
	...
	var arControls = [
		[
			...
			<?php  foreach ($arSites as $SITE_ID => $SITE_NAME) { ?>, document.forms.intr_opt_form.iblock_absence_<?=$SITE_ID; ?><?php  } ?>
		],
		[
			<?php $i = 0; foreach ($arSites as $SITE_ID => $SITE_NAME) echo ($i++<=0?'':', ').'document.forms.intr_opt_form.iblock_vacancy_'.$SITE_ID;?>
		],
		[
			<?php $i = 0; foreach ($arSites as $SITE_ID => $SITE_NAME) echo ($i++<=0?'':', ').'document.forms.intr_opt_form.iblock_calendar_'.$SITE_ID;?>,
			<?php $i = 0; foreach ($arSites as $SITE_ID => $SITE_NAME) echo ($i++<=0?'':', ').'document.forms.intr_opt_form.iblock_group_calendar_'.$SITE_ID;?>
		]
	];
	...
	for (var i = 0; i < arControls[index].length; i++)
	{
		...
		{
			arControls[index][i].options[0] = new Option('<?php echo GetMessage('INTR_CHOOSE_IBTYPE')?>', '');
		}
	}
}
...
{
<?php foreach ($arSites as $SITE_ID => $SITE_NAME):?>document.getElementById(type + '_site_row_<?php echo $SITE_ID?>').style.display = 'none';<?php endforeach;?>
	document.getElementById(type + '_site_row_' + site).style.display = '';
}
</script>
<form method="post" name="intr_opt_form" action="<?php echo $APPLICATION->GetCurPage()?>?mid=<?=urlencode($mid)?>&amp;lang=<?php echo LANGUAGE_ID?>">
...
<table width="100%" align="center">
	<tr class="heading">
		<td colspan="2"><?php echo GetMessage('INTR_OPT_SECTION_IB_STRUCTURE')?></td>
	</tr>
	<tr>
		<td valign="top" width="50%"><?php echo GetMessage('INTR_OPTION_IBLOCK_TYPE')?>:</td>
		<td valign="top" width="50%"><select name="IBLOCK_TYPE" onchange="change_iblock_list(this.value)">
			<option value=""><?php echo GetMessage('INTR_OPTION_NOT_SET')?></option>
			<?php  foreach ($arIBTypes as $ibtype_id => $ibtype_name): ?>
				<option value="<?=$ibtype_id ?>" <?php  if ($ibtype_id == $current_ibtype) echo 'selected'; ?>><?=htmlspecialcharsbx($ibtype_name) ?></option>
			<?php  endforeach ?>
		</select></td>
	</tr>
	<tr>
		<td valign="top" width="50%"><?php echo GetMessage('INTR_OPTION_IBLOCK_STRUCTURE')?>:</td>
		<td><select name="IBLOCK_STRUCTURE">
<?php 
if ($current_ib_structure || $current_ibtype):
?>
	<?php foreach ($arIB[$current_ibtype] as $iblock_id => $iblock):?><option value="<?php echo $iblock_id?>"<?php echo $iblock_id == $current_ib_structure ? ' selected="selected"' : ''?>><?=htmlspecialcharsbx($iblock) ?></option><?php endforeach;?>
<?php 
else:
?>
	<option value=""><?php echo GetMessage('INTR_CHOOSE_IBTYPE')?></option>
<?php 
endif;
?></select></td>
	</tr>
	<tr>
		<td valign="top" width="50%"><?php echo GetMessage('INTR_OPTION_IBLOCK_HONOUR')?>:</td>
		<td><select name="IBLOCK_HONOUR">
<?php 
if ($current_ib_honour || $current_ibtype):
?>
	<?php foreach ($arIB[$current_ibtype] as $iblock_id => $iblock):?><option value="<?php echo $iblock_id?>"<?php echo $iblock_id == $current_ib_honour ? ' selected="selected"' : ''?>><?=htmlspecialcharsbx($iblock) ?></option><?php endforeach;?>
<?php 
else:
?>
	<option value=""><?php echo GetMessage('INTR_CHOOSE_IBTYPE')?></option>
...
?></select></td>
	...
	<tr>
		<td valign="top" width="50%"><?php echo GetMessage('INTR_OPTION_IBLOCK_STATE_HISTORY')?>:</td>
		<td><select name="IBLOCK_STATE_HISTORY">
<?php 
if ($current_ib_state_history || $current_ibtype):
?>
	<?php foreach ($arIB[$current_ibtype] as $iblock_id => $iblock):?><option value="<?php echo $iblock_id?>"<?php echo $iblock_id == $current_ib_state_history ? ' selected="selected"' : ''?>><?=htmlspecialcharsbx($iblock) ?></option><?php endforeach;?>
<?php 
else:
?>
	<option value=""><?php echo GetMessage('INTR_CHOOSE_IBTYPE')?></option>
...
?></select></td>
	...
	<tr>
		...
		<td valign="top" width="50%"><select name="IBLOCK_TYPE_VACANCY" onchange="change_iblock_list(this.value, 1)">
			...
			<?php  foreach ($arIBTypes as $ibtype_id => $ibtype_name): ?>
				<option value="<?=$ibtype_id ?>" <?php  if ($ibtype_id == $current_ibtype_vacancy) echo 'selected'; ?>><?=htmlspecialcharsbx($ibtype_name) ?></option>
			<?php  endforeach ?>
		</select></td>
	</tr>
	<?php  if (COption::GetOptionString("intranet", "calendar_2", "N") != "Y" || !CModule::IncludeModule('calendar')) { ?>
	<tr class="heading">
		<td colspan="2"><?php echo GetMessage('INTR_OPT_SECTION_IB_CALENDAR')?></td>
	</tr>
	<tr>
		<td valign="top" width="50%"><?php echo GetMessage('INTR_OPTION_IBLOCK_TYPE')?>:</td>
		<td valign="top" width="50%"><select name="IBLOCK_TYPE_CALENDAR" onchange="change_iblock_list(this.value, 2)">
			<option value=""><?php echo GetMessage('INTR_OPTION_NOT_SET')?></option>
			<?php  foreach ($arIBTypes as $ibtype_id => $ibtype_name): ?>
				<option value="<?=$ibtype_id ?>" <?php  if ($ibtype_id == $current_ibtype_calendar) echo 'selected'; ?>><?=htmlspecialcharsbx($ibtype_name) ?></option>
			<?php  endforeach ?>
		</select></td>
	</tr>
	<?php  } ?>
	<tr class="heading">
		<td colspan="2"><?php echo GetMessage('INTR_OPT_OTHER')?></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td valign="top" width="50%">
			...
			<?php foreach($vacationTypes as $type):?>
...
<table width="100%" align="center">
	...
	<tr id="iblock_absence_site_row_<?=$SITE_ID?>">
		<td valign="top" width="50%"><?php echo GetMessage('INTR_OPTION_IBLOCK_ABSENCE')?>:</td>
		<td><select name="iblock_absence_<?php echo $SITE_ID?>">
		<?php 
		if ($current_ibtype):
			?>
			<?php foreach ($arIB[$current_ibtype] as $iblock_id => $iblock):?><option value="<?php echo $iblock_id?>"<?php echo $iblock_id == $iblock_absence[$SITE_ID] ? ' selected="selected"' : ''?>><?=htmlspecialcharsbx($iblock) ?></option><?php endforeach;?>
			<?php 
		else:
			?>
			<option value=""><?php echo GetMessage('INTR_CHOOSE_IBTYPE')?></option>
	...
	<tr id="iblock_vacancy_site_row_<?=$SITE_ID; ?>">
		...
		<?php  if ($current_ibtype_vacancy) { ?>
			<?php  foreach ($arIB[$current_ibtype_vacancy] as $iblock_id => $iblock) { ?>
			<option value="<?=$iblock_id; ?>"<?php  if ($iblock_id == $iblock_vacancy[$SITE_ID]) { ?> selected="selected"<?php  } ?>>
				<?=$iblock; ?>
			</option>
			<?php  } ?>
		<?php  } else { ?>
	...
	<tr id="iblock_calendar_site_row_<?=$SITE_ID?>">
		<td valign="top" width="50%"><?php echo GetMessage('INTR_OPTION_IBLOCK_CALENDAR')?>:</td>
		<td><select name="iblock_calendar_<?php echo $SITE_ID?>">
		<?php 
		if ($current_ibtype):
			?>
			<?php foreach ($arIB[$current_ibtype_calendar] as $iblock_id => $iblock):?><option value="<?php echo $iblock_id?>"<?php echo $iblock_id == $iblock_calendar[$SITE_ID] ? ' selected="selected"' : ''?>><?=htmlspecialcharsbx($iblock) ?></option><?php endforeach;?>
			<?php 
		else:
			?>
			<option value=""><?php echo GetMessage('INTR_CHOOSE_IBTYPE')?></option>
	...
	<tr id="iblock_group_calendar_site_row_<?=$SITE_ID?>">
		<td valign="top" width="50%"><?php echo GetMessage('INTR_OPTION_IBLOCK_GROUP_CALENDAR')?>:</td>
		<td><select name="iblock_group_calendar_<?php echo $SITE_ID?>">
		<?php 
		if ($current_ibtype):
			?>
			<?php foreach ($arIB[$current_ibtype_calendar] as $iblock_id => $iblock):?><option value="<?php echo $iblock_id?>"<?php echo $iblock_id == $iblock_group_calendar[$SITE_ID] ? ' selected="selected"' : ''?>><?=htmlspecialcharsbx($iblock) ?></option><?php endforeach;?>
			<?php 
		else:
			?>
			<option value=""><?php echo GetMessage('INTR_CHOOSE_IBTYPE')?></option>
	...
	if (!IsModuleInstalled('extranet') || $SITE_ID != COption::GetOptionString("extranet", "extranet_site")):
		...
		<tr id="block_new_user_lf_site_row_<?=$SITE_ID?>">
			<td valign="top" width="50%"><?php echo GetMessage("INTR_OPTION_BLOCK_NEW_USER_LF")?></td>
			<td valign="top" width="50%"><input type="checkbox" name="BLOCK_NEW_USER_LF_SITE_<?php echo $SITE_ID?>" value="Y" <?php echo ($BLOCK_NEW_USER_LF_SITE[$SITE_ID] == "Y" ? " checked" : "")?> /></td>
		</tr>
		<?php 
	endif;
	?>
...
<?php  foreach ($arParamsGroup as $group => $arParams) { ?>

<tr class="heading">
	<td colspan="2"><?=GetMessage('INTR_SETTINGS_COMPONENT_'.$group); ?></td>
</tr>

	<?php  foreach ($arParams as $key => $arParam) { ?>
	<?php 
	...
	<tr>
		...
		<?php  switch ($arParam['TYPE'])
		{
			case 'CHECKBOX': ?>
				<input type="hidden" name="IMPORT[<?=$key?>]" value="N" />
				<input type="checkbox" name="IMPORT[<?=$key?>]" value="Y"<?php  if ($value == 'Y') { ?> checked="checked"<?php  } ?> />
				<?php  break;
			...
			case 'LIST':
				...
				<select name="IMPORT[<?=$key; ?>]<?php  if ($bMultiple) { ?>[]<?php  } ?>"<?php  if ($bMultiple) { ?> multiple="multiple" size="10"<?php  } ?>>
				<?php  foreach ($arParam['VALUES'] as $val => $title) { ?>
				<option value="<?=htmlspecialcharsbx($val); ?>"<?php  if (in_array($val, (array) $value)) { ?> selected="selected"<?php  } ?>><?=htmlspecialcharsbx($title); ?></option>
				<?php  } ?>
				</select>
				<?php  break;
...
function disable_tz_controls(obCheckbox)
{
	...
	{
		<?php foreach ($arVariants as $var):?>
		obForm.tz_transition_<?php echo $var;?>.disabled = true;
		obForm.tz_transition_<?php echo $var;?>_tpl.disabled = true;
		<?php endforeach?>
	}
	else
	{
		<?php foreach ($arVariants as $var):?>
		obForm.tz_transition_<?php echo $var;?>.disabled = false;
		obForm.tz_transition_<?php echo $var;?>_tpl.disabled = false;
		<?php endforeach?>
	}
}
</script>
	<tr>
		<td valign="top" width="45%"><?php echo GetMessage('INTR_OPTION_GET_CONTACTS_IMAGES')?>: </td>
		<td width="55%">
			<input type="checkbox" name="ws_contacts_get_images" id="ws_contacts_get_images" value="Y" <?php echo $ws_contacts_get_images == 'Y' ? 'checked="checked"' : ''?> /><label for="ws_contacts_get_images"><?php echo GetMessage('INTR_OPTION_WS_CONTACTS_GET_IMAGES_YES')?></label>
		</td>
	</tr>
	<tr>
		<td valign="top" width="45%"><?php echo GetMessage('INTR_OPTION_FIRSTDAY')?>: </td>
		<td width="55%"><select name="first_week_day">
<?php 
foreach ($arWeekDays as $day):
?>
			<option value="<?php echo $day?>"<?php echo $first_week_day == $day ? ' selected="selected"' : ''?>><?php echo GetMessage('INTR_OPTION_FIRSTDAY_'.$day);?></option>
<?php 
endforeach;
?>
		</select></td>
	<tr class="heading">
		<td colspan="2"><?php echo GetMessage('INTR_OPTION_TZ')?></td>
	</tr>
	<tr>
		<td valign="top"><?php echo GetMessage('INTR_OPTION_TZ_USE_DAYLIGHT_SAVING')?>: </td>
		<td><input type="checkbox" name="tz_transition" id="tz_transition" <?php echo $tz_transition == 'N' ? '' : 'checked="checked"'?> value="Y" onclick="disable_tz_controls(this)" /><label for="tz_transition"><?php echo GetMessage('INTR_OPTION_TZ_USE_DAYLIGHT_SAVING_YES')?></label></td>
	</tr>
<?php 
foreach ($arVariants as $var):
?>
	<tr>
		<td valign="top"><?php echo GetMessage('INTR_OPTION_TZ_TO_'.strtoupper($var).'_DATE')?>: </td>
		<td>
			<select name="tz_transition_<?php echo $var?>_tpl" onchange="check_other(this, '<?php echo $var?>')" <?php echo $tz_transition == 'N' ? 'disabled="disabled"' : ''?>>
				<option value=""><?php echo GetMessage('INTR_OPTION_NOT_SET')?></option>
<?php 
$bFound = false;
$firstRule = '';
...
?>
	<option value="<?php echo htmlspecialcharsbx($rule)?>" <?php echo ($rule === ${'tz_transition_'.$var} || (!$bFound && $rule == 'other' && ${'tz_transition_'.$var} != '')) ? 'selected="selected"' : ''?>><?php echo GetMessage('INTR_OPTION_TZ_TO_'.strtoupper($var).'_DATE_'.$id)?></option>
<?php 
endforeach;
?>
			</select><br />
			<div id="tz_edit_<?php echo $var;?>" style="display: <?php echo $bFound || ${'tz_transition_'.$var} == '' ? 'none' : 'block'?>">
				<textarea name="tz_transition_<?php echo $var?>" cols="40" rows="5" <?php echo $tz_transition == 'N' ? 'disabled="disabled"' : ''?>><?php echo htmlspecialcharsbx(${'tz_transition_'.$var});?></textarea><br />
				<small><?php echo GetMessage('INTR_OPTION_TZ_HINT');?></small>
			</div>
		</td>
	</tr>
<?php 
...
?>
	<tr>
		<td valign="top"><?php echo GetMessage('INTR_OPTION_TZ_SERVER')?>: </td>
		<td>
			<?php echo GetMessage('INTR_OPTION_TZ_SERVER_GMT_DIFF')?>: <?php echo date('O')?> (<?php echo date('Z')?>)<br />
			<?php echo GetMessage('INTR_OPTION_TZ_SERVER_DAYLIGHT_SAVING')?>: <?php echo date('I') ? GetMessage('INTR_OPTION_TZ_SERVER_DAYLIGHT_SAVING_1') : GetMessage('INTR_OPTION_TZ_SERVER_DAYLIGHT_SAVING_0');?><br />
			<?php echo GetMessage('INTR_OPTION_TZ_SERVER_CURRENT_RFC_2822')?>: <?php echo date('r');?>
		</td>
	</tr>

	<tr class="heading">
		<td colspan="2"><?php echo GetMessage('INTR_OPTION_PATHS')?></td>
	</tr>
	<tr>
		<td colspan="2">
<?php 
...
?>
	<table width="100%" align="center">
	<tr>
		<td valign="top" align="right"><?php echo GetMessage('INTR_OPTION_PATHS_USER')?>: </td>
		<td><input type="text" name="path_user_<?=$SITE_ID?>" value="<?php echo htmlspecialcharsbx($path_user[$SITE_ID])?>" size="50" /></td>
	</tr>
	<tr>
		<td valign="top" align="right"><?php echo GetMessage('INTR_OPTION_PATHS_TASK_USER')?>: </td>
		<td><input type="text" name="path_task_user_<?=$SITE_ID?>" value="<?php echo htmlspecialcharsbx($path_task_user[$SITE_ID])?>" size="50" /></td>
	</tr>
	<tr>
		<td valign="top" align="right"><?php echo GetMessage('INTR_OPTION_PATHS_TASK_USER_ENTRY')?>: </td>
		<td><input type="text" name="path_task_user_entry_<?=$SITE_ID?>" value="<?php echo htmlspecialcharsbx($path_task_user_entry[$SITE_ID])?>" size="50" /></td>
	</tr>
	<tr>
		<td valign="top" align="right"><?php echo GetMessage('INTR_OPTION_PATHS_TASK_GROUP')?>: </td>
		<td><input type="text" name="path_task_group_<?=$SITE_ID?>" value="<?php echo htmlspecialcharsbx($path_task_group[$SITE_ID])?>" size="50" /></td>
	</tr>
	<tr>
		<td valign="top" align="right"><?php echo GetMessage('INTR_OPTION_PATHS_TASK_GROUP_ENTRY')?>: </td>
		<td><input type="text" name="path_task_group_entry_<?=$SITE_ID?>" value="<?php echo htmlspecialcharsbx($path_task_group_entry[$SITE_ID])?>" size="50" /></td>
	</tr>
	</table>
<?php 
endforeach;
...
<?php /*	<tr>
		<td><?php echo GetMessage("INTR_OPTION_SEARCH_USER_URL");?>:</td><td><input type="text" size="47" name="search_user_url" value="<?php echo htmlspecialcharsbx(COption::GetOptionString("intranet", "search_user_url"))?>"></td>
	</tr>*/?>
	<tr class="heading">
		<td colspan="2"><?php echo GetMessage("INTR_OPTION_SEARCH_CONVERTERS");?></td>
	</tr>
	<?php if(!function_exists("exec")):?>
	<tr>
		<td colspan="2"><?php echo GetMessage("INTR_OPTION_SEARCH_CONVERTERS_WARNING");?></td>
	</tr>
	<?php endif?>
	<?php if(!function_exists("zip_open")):?>
	<tr>
		<td colspan="2"><?php echo GetMessage("INTR_OPTION_SEARCH_ZIP_EXTENTION_WARNING");?></td>
	</tr>
	<?php endif?>
	<tr>
	<td colspan="2">
		...
		<tr class="heading">
			<td><?php echo GetMessage("INTR_OPTION_SEARCH_CONVERTER_EXTENSION");?></td>
			<td><?php echo GetMessage("INTR_OPTION_SEARCH_CONVERTER_PROGRAM");?></td>
			<td><?php echo GetMessage("INTR_OPTION_SEARCH_CONVERTER_DIRECTORY");?></td>
		</tr>
		<?php $arExt = explode(",", COption::GetOptionString("intranet", "search_file_extensions"));
		$i = 0;
		foreach($arExt as $ext):?>
			<tr>
				<td><input type="text" size="5" name="search_file_extension_<?php echo $i?>" value="<?php echo htmlspecialcharsbx($ext)?>"></td>
				<td><input type="text" size="50" name="search_file_extension_exe_<?php echo $i?>" value="<?php echo htmlspecialcharsbx(COption::GetOptionString("intranet", "search_file_extension_exe_".$ext))?>"></td>
				<td><input type="text" size="25" name="search_file_extension_cd_<?php echo $i?>" value="<?php echo htmlspecialcharsbx(COption::GetOptionString("intranet", "search_file_extension_cd_".$ext))?>"></td>
			</tr>
		<?php 
		$i++;
		endforeach;
		for($j = 0; $j < 5; $j++):?>
			<tr>
				<td><input type="text" size="5" name="search_file_extension_<?php echo $i?>" value=""></td>
				<td><input type="text" size="50" name="search_file_extension_exe_<?php echo $i?>" value=""></td>
				<td><input type="text" size="25" name="search_file_extension_cd_<?php echo $i?>" value=""></td>
			</tr>
		<?php 
		$i++;
		endfor?>
...
<?php $tabControl->BeginNextTab();?>
	...
	<tr>
		...
		<td>
			<input type="checkbox" id="allow_external_mail" name="allow_external_mail" value="Y"<?php  if ($allow_external_mail == 'Y') { ?> checked="checked"<?php  } ?> />
	...
	<tr>
		<td colspan="2">
			<?php  $childTabControl_2->begin(); ?>
			<?php  foreach ($arSites as $SITE_ID => $SITE): ?>
				<?php  if (isModuleInstalled('extranet') && $SITE_ID == \COption::getOptionString('extranet', 'extranet_site')) continue; ?>
				<?php  $childTabControl_2->beginNextTab(); ?>
...
<?php $tabControl->Buttons();?>
	...
	<?php endif?>
	<input type="submit" name="RestoreDefaults" title="<?php echo GetMessage("MAIN_HINT_RESTORE_DEFAULTS")?>" onclick="return confirm('<?php echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>')" value="<?php echo GetMessage("INTR_RESTORE_DEFAULTS")?>">
<?php $tabControl->End();?>
</form>