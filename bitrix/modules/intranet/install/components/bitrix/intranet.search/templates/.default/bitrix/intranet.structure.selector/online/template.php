...
if ($arParams['AJAX_MODE'] == 'Y')
{
	...
	$onclick = 'BX.ajax.insertToNode(\''.CUtil::JSEscape($url).'\' + (this.checked ? \''.CUtil::JSEscape('&'.$arParams['FILTER_NAME'].'_IS_ONLINE=Y').'\' : \'\'), \'comp_'.$arParams['AJAX_ID'].'\', '.($arParams['AJAX_OPTION_SHADOW'] == 'Y' ? 'true' : 'false').');';
}
else
{
...
?>
<input type="checkbox" name="<?php echo $arParams['FILTER_NAME'].'_IS_ONLINE'?>" value="Y" <?php echo $bChecked ? 'checked="checked"' : ''?> id="<?php echo $arParams['FILTER_NAME'].'_IS_ONLINE'?>" title="<?php echo GetMessage('ISS_TPL_ONLINE_TITLE')?>" onclick="<?php echo $onclick?>" />
