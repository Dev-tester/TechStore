...
<div class="post-dialog-wrap<?php  if (IsIE() == 8) { ?> bx-ie bx-ie8<?php  } ?>">

<?php  if ($arResult['STEP'] == 'choose') { ?>
...
{
	$k++;
	?><a href="?STEP=setup&SERVICE=<?=$id; ?>" class="post-dialog-service-item post-dialog-service-name"<?php  if (strlen($settings['name']) > 15) { ?> style="font-size: 18px; "<?php  } ?>>
		...
		&nbsp;<?=$settings['name']; ?>&nbsp;
		<?php  } ?>
	</a><?php  if ($k%2 == 0) { ?><div class="post-dialog-border"></div><?php  }
} ?>
<?php  if ($k%2 != 0) { ?><div class="post-dialog-border"></div><?php  } ?><br><br>

<?php  } else if ($arResult['STEP'] == 'setup') { ?>

<table class="post-dialog-title">
	<tr>
		<td class="post-dialog-title-text">
			<?php  if (!$arParams['SERVICES'][$arResult['SERVICE']]['icon']) { ?>
...
<form id="settings_form" name="settings_form" action="<?=POST_FORM_ACTION_URI; ?>" method="POST">
	...
	<?php  if (empty($arParams['SERVICES'][$arResult['SERVICE']][$option])) { ?>
	<?php  switch ($option) {
		case 'server': ?>
			<div class="post-dialog-inp-item post-dialog-inp-serv">
				<span class="post-dialog-inp-label"><?=GetMessage('INTR_MAIL_INP_SERVER'); ?></span>
				<input id="server" name="server" type="text" class="post-dialog-inp"<?php  if (!empty($arResult['SETTINGS']['server'])) { ?> value="<?=htmlspecialcharsbx($arResult['SETTINGS']['server']); ?>"<?php  } ?>>
			</div>
			<?php  break;
		case 'port': ?>
			<div class="post-dialog-inp-item post-dialog-inp-post">
				<span class="post-dialog-inp-label"><?=GetMessage('INTR_MAIL_INP_PORT'); ?></span>
				<input id="port" name="port" type="text" class="post-dialog-inp"<?php  if (!empty($arResult['SETTINGS']['port'])) { ?> value="<?=htmlspecialcharsbx($arResult['SETTINGS']['port']); ?>"<?php  } ?>>
			</div>
			<?php  break;
		case 'encryption': ?>
			<div class="post-dialog-inp-item">
				...
				<span class="post-dialog-inp-select-wrap">
					<select name="encryption" class="post-dialog-inp-select">
						<option value="Y"<?php  if (empty($arResult['SETTINGS']['encryption']) || $arResult['SETTINGS']['encryption'] != 'N') { ?> selected="selected"<?php  } ?>><?=GetMessage('INTR_MAIL_INP_ENCRYPT_YES'); ?></option>
						<option value="N"<?php  if (isset($arResult['SETTINGS']['encryption']) && $arResult['SETTINGS']['encryption'] == 'N') { ?> selected="selected"<?php  } ?>><?=GetMessage('INTR_MAIL_INP_ENCRYPT_NO'); ?></option>
					</select>
				</span>
			</div>
			<?php  break;
		case 'link': ?>
			<div class="post-dialog-inp-item">
				...
				<input id="link" name="link" type="text" class="post-dialog-inp"<?php  if (!empty($arResult['SETTINGS']['link'])) { ?> value="<?=htmlspecialcharsbx($arResult['SETTINGS']['link']); ?>"<?php  } ?>/>
			</div>
			<?php  break;
		case 'login': ?>
			<div class="post-dialog-inp-item">
				<span class="post-dialog-inp-label"><?=GetMessage('INTR_MAIL_INP_LOGIN'); ?></span>
				<input name="login" type="text" class="post-dialog-inp"<?php  if (!empty($arResult['SETTINGS']['login'])) { ?> value="<?=htmlspecialcharsbx($arResult['SETTINGS']['login']); ?>"<?php  } ?>>
			</div>
			<?php  break;
		case 'password': ?>
			<div class="post-dialog-inp-item">
...
<?php  } else if ($arResult['STEP'] == 'confirm') { ?>
...
<table class="post-dialog-success-table">
	<tr>
		<td class="post-dialog-success-cell">
			...
			<br><br><br>
			<a class="post-dialog-success-link" href="<?=$APPLICATION->GetCurPage(); ?>" <?php  if ($arResult['ACT'] != 'remove') { ?> target="_blank"<?php  } ?>>
