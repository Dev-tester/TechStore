	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="intask-main data-table">
		...
		<tbody>
			<tr class="intask-row">
				...
				<td class="intask-cell">
					<select name="flt_id">
						...
						<?php foreach ($arResult["MEETINGS_ALL"] as $key => $value):?>
				...
				<td class="intask-cell">
					<select name="flt_duration">
						...
						for ($i = 0; $i < 12; $i++)
						{
							if ($i > 0)
							{
								?><option value="<?= $i ?>.0"<?= ($_REQUEST["flt_duration"] == $i ? " selected" : "") ?>><?= $i ?>.0</option><?php 
							}
							?><option value="<?= $i ?>.5"<?= ($_REQUEST["flt_duration"] == $i.".5" ? " selected" : "") ?>><?= $i ?>.5</option><?php 
						}
						?>
					</select>
				</td>
			...
			<tr class="intask-row">
				...
				<td class="intask-cell">
					<input type="text" name="flt_uf_place" <?php  if (is_scalar($_REQUEST['flt_uf_place'])): ?>value="<?= HtmlSpecialCharsbx($_REQUEST["flt_uf_place"]) ?>"<?php  endif ?> size="5">
				</td>
			</tr>
			<tr class="intask-row">
