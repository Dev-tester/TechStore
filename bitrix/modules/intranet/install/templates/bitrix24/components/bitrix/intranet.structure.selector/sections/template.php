...
if (count($arResult['SECTIONS_CHAIN']) > 0):?>
	<div class="breadcrumbs">
		<a class="breadcrumbs-home" href="<?=$arParams["VIS_STRUCTURE_URL"]?>"><i></i></a><?php 
		$last_section_key = count($arResult['SECTIONS_CHAIN']) - 1; 
		foreach ($arResult['SECTIONS_CHAIN'] as $key => $arItem)
		{    
			if ($key != $last_section_key):?><a class="breadcrumbs-item" href="<?=$arItem[2]?>"><?=$arItem[1]?><i></i></a><?php else:?><span class="breadcrumbs-item-selected"><?=$arItem[1]?></span><?php endif;			
		}
	?></div>
<?php endif;
if (intval($arResult["SECTION_DATA"]["IBLOCK_SECTION_ID"]) > 0):?>
	<div class="department-unit-manager">
		...
		<?php if (intval($arResult["SECTION_DATA"]["IBLOCK_SECTION_UF_HEAD"]) > 0):?>
...
<div class="department-description">
	...
	<?php echo ($arResult['SECTION_DATA']['DESCRIPTION_TYPE'] == 'text' ? htmlspecialcharsex($arResult['SECTION_DATA']['DESCRIPTION']) : $arResult['SECTION_DATA']['DESCRIPTION'])?>
	</div>
</div>
<?php endif;
...
<div class="department-subdivision-list">
	...
	<?php foreach ($arResult['SECTIONS'] as $key => $arSect):?>
		<div class="">
...
?>  
	<div class="department-employee-list">
		<div class="department-titles"><?=GetMessage("INTR_IS_TPL_EMPLOYEES")?><?php echo " (".$arResult['SECTION_DATA']["USER_COUNT"].")"?></div>
	<?php 
