...
<form method="post" action="<?php $APPLICATION->GetCurPage()?>" name="form1">
...
<input type="hidden" name="BASE" value="<?php echo htmlspecialcharsbx($currency['BASE']); ?>">
<?php 
if (isset($return_url) && $return_url != '')
{
	?><input type="hidden" name="return_url" value="<?=htmlspecialcharsbx($return_url)?>"><?php 
}

$tabControl->Begin();?>
<?php $tabControl->BeginNextTab();?>
	<tr class="adm-detail-required-field">
		<td width="40%"><?php echo GetMessage("currency_curr")?>:</td>
		<td width="60%">
		<?php if (!$ID):?>
			<input type="text" value="<?php echo htmlspecialcharsbx($currency['CURRENCY']);?>" size="3" name="CURRENCY" maxlength="3">
		<?php else:?>
	...
	<tr class="adm-detail-required-field">
		<td width="40%"><?php  echo GetMessage("currency_base"); ?>:</td>
		<td width="60%"><?php  echo ($currency['BASE'] == 'Y' ? GetMessage('BASE_CURRENCY_YES') : GetMessage('BASE_CURRENCY_NO')); ?></td>
	</tr>
	<tr class="adm-detail-required-field">
		<td width="40%"><?php echo GetMessage("currency_rate_cnt")?>: <span class="required" style="vertical-align: super; font-size: smaller;">1</span></td>
		<td width="60%">
			<input type="text" size="10" name="AMOUNT_CNT" value="<?=(int)$currency['AMOUNT_CNT']; ?>"<?php  echo ($currency['BASE'] == 'Y' ? ' disabled' : ''); ?>>
		</td>
	</tr>
	<tr class="adm-detail-required-field">
		<td width="40%"><?php echo GetMessage("currency_rate")?>: <span class="required" style="vertical-align: super; font-size: smaller;">1</span></td>
		<td width="60%">
			<input type="text" size="20" name="AMOUNT" value="<?=htmlspecialcharsbx($currency['AMOUNT'])?>" maxlength="20"<?php  echo ($currency['BASE'] == 'Y' ? ' disabled' : ''); ?>>
		</td>
	</tr>
	<tr>
		<td width="40%"><?php echo GetMessage("currency_numcode")?>:</td>
		<td width="60%">
			<input type="text" size="3" name="NUMCODE" value="<?php echo htmlspecialcharsbx($currency['NUMCODE']); ?>" maxlength="3">
		</td>
	</tr>
	<tr>
		<td width="40%"><?php echo GetMessage("currency_sort_ex")?>:</td>
		<td width="60%">
			<input type="text" size="10" name="SORT" value="<?php echo (int)$currency['SORT']; ?>" maxlength="10">
		</td>
	</tr>
<?php $tabControl->BeginNextTab();
	foreach ($currencyLangs as $languageId => $settings)
	{
		...
		<tr>
			<td width="40%"><?php echo GetMessage("CURRENCY_FULL_NAME")?>:</td>
		...
		<tr>
			<td width="40%"><?php echo GetMessage("CURRENCY_FORMAT_TEMPLATE")?>:</td>
			<td width="60%">
				<select name="format_<?=htmlspecialcharsbx($languageId); ?>" onchange="setTemplate('<?=$scriptLanguageId; ?>')">
					...
					<?php foreach ($arTemplates as $key => $ar):?>
		...
		<tr class="adm-detail-required-field">
			<td width="40%"><?php echo GetMessage("CURRENCY_FORMAT_DESC")?>:</td>
		...
		<tr>
			<td width="40%"><?php echo GetMessage("CURRENCY_DEC_POINT_DESC")?>:</td>
		...
		<tr>
			<td width="40%"><?php echo GetMessage("THOU_SEP_DESC")?>:</td>
			<td width="60%">
				...
				?>
				<option value=""<?php  echo ($settings['THOUSANDS_VARIANT'] == '' && $settings['THOUSANDS_SEP'] != '' ? ' selected' : '');?>><?=htmlspecialcharsbx(GetMessage("CURRENCY_THOUSANDS_VARIANT_O")); ?></option>
				</select>
		...
		<tr>
			<td width="40%"><?php echo GetMessage("DECIMALS_DESC")?>: <span class="required" style="vertical-align: super; font-size: smaller;">2</span></td>
		...
		<tr>
			<td width="40%"><?php  echo GetMessage('HIDE_ZERO_DECIMALS'); ?>: <span class="required" style="vertical-align: super; font-size: smaller;">3</span></td>
			<td width="60%">
				<input type="hidden" name="<?=$fieldPrefix; ?>[HIDE_ZERO]" value="N">
				<input type="checkbox" name="<?=$fieldPrefix; ?>[HIDE_ZERO]" value="Y" <?php  echo ($settings['HIDE_ZERO'] == 'Y' ? 'checked' : ''); ?>>
			</td>
		</tr>
		<?php 
