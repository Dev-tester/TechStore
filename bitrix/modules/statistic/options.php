...
if ($STAT_RIGHT>="R"):
	...
	$tabControl->Begin();
	?>
	<form name="form_settings" method="POST" action="<?php echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialcharsbx($mid)?>&amp;lang=<?=LANGUAGE_ID?>">
	<?php $tabControl->BeginNextTab();?>
	<tr>
		<td width="40%">
			<?php echo GetMessage("STAT_OPT_DAILY_REPORT_TIME2")?>
		</td>
	...
	foreach($arOPTIONS["TAB1"] as $key => $Option):
		if (!is_array($Option)):
			if($key == "USE_AUTO_OPTIMIZE" && strtolower($statDB->type) == "mysql"):?>
				<tr>
					<td><label for="<?=$key?>"><?php echo GetMessage("STAT_OPT_USE_AUTO_OPTIMIZE")?></label></td>
					<td nowrap><input type="checkbox" name="<?=$key?>" id="<?=$key?>" value="Y" <?php if(${$key}=="Y") echo "checked";?>></td>
				</tr>
			<?php elseif($key == "BASE_CURRENCY" && CModule::IncludeModule("currency")):?>
				<tr>
					<td class="adm-detail-valign-top"><?php echo GetMessage("STAT_OPT_BASE_CURRENCY")?></td>
					<td><?php echo CCurrency::SelectBox("BASE_CURRENCY", $BASE_CURRENCY, " ", True, "") ?><br>
					<input type="checkbox" name="recount_base_currency" id="recount_base_currency" value="Y"><label for="recount_base_currency"><?php echo GetMessage("STAT_OPT_DO_RECOUNT")?></label></td>
				</tr>
			<?php endif;
		else:
			...
			?>
			<tr>
				<td <?php if($type[0]=="textarea") echo 'class="adm-detail-valign-top"'?>><label for="<?php echo htmlspecialcharsbx($Option[0])?>"><?php echo $Option[1]?></label></td>
				<td nowrap>
	...
	endforeach;?>
	<tr class="heading">
		<td align="center" colspan="2" nowrap><?php echo GetMessage("STAT_OPT_GRABBER_DEFENCE_SECTION")?></td>
	</tr>
	<?php if (CModule::IncludeModule("fileman")):?>
	<tr>
		<td align="center" colspan=2>[ <a href="/bitrix/admin/fileman_file_edit.php?lang=<?=LANGUAGE_ID?>&amp;full_src=Y&amp;path=%2Fbitrix%2Factivity_limit.php"><?php echo GetMessage("STAT_OPT_GRABBER_EDIT_503_TEMPLATE_LINK")?></a> ]</td>
	</tr>
	<?php endif;?>
	<tr>
		<td nowrap><label for="DEFENCE_ON"><?php echo GetMessage("STAT_OPT_DEFENCE_ON")?></label></td>
		<td><?php echo InputType("checkbox","DEFENCE_ON","Y",$DEFENCE_ON,false,"","OnClick=\"ChangeDefenceSwitch()\" id=\"DEFENCE_ON\"")?></td>
	</tr>
	<tr>
		<td nowrap><?php echo GetMessage("STAT_OPT_DEFENCE_DELAY")?></td>
		<td><input size="3" type="text" name="DEFENCE_DELAY" id="DEFENCE_DELAY" value="<?=htmlspecialcharsbx($DEFENCE_DELAY)?>">&nbsp;<?php echo GetMessage("STAT_OPT_DEFENCE_DELAY_MEASURE_SEC")?></td>
	</tr>
	<tr>
		<td nowrap><?php echo GetMessage("STAT_OPT_DEFENCE_STACK_TIME")?></td>
		<td><input size="3" type="text" name="DEFENCE_STACK_TIME" id="DEFENCE_STACK_TIME" value="<?=htmlspecialcharsbx($DEFENCE_STACK_TIME)?>">&nbsp;<?php echo GetMessage("STAT_OPT_DEFENCE_STACK_TIME_MEASURE_SEC")?></td>
	</tr>
	<tr>
		<td nowrap><?php echo GetMessage("STAT_OPT_DEFENCE_MAX_STACK_HITS")?></td>
		<td><input size="3" type="text" name="DEFENCE_MAX_STACK_HITS" id="DEFENCE_MAX_STACK_HITS" value="<?=htmlspecialcharsbx($DEFENCE_MAX_STACK_HITS)?>">&nbsp;<?php echo GetMessage("STAT_OPT_DEFENCE_MAX_STACK_HITS_MEASURE")?></td>
	</tr>
	<tr>
		<td nowrap><label for="DEFENCE_LOG"><?php echo GetMessage("STAT_OPT_DEFENCE_LOG", array("#HREF#"=>"/bitrix/admin/event_log.php?lang=".LANGUAGE_ID."&set_filter=Y&find_type=audit_type_id&find_audit_type[]=STAT_ACTIVITY_LIMIT"))?></label></td>
	...
	foreach($arOPTIONS["TAB5"] as $key => $Option):
		if(!is_array($Option)):
			if($key == "ADV_EVENTS_DEFAULT"):?>
				<tr>
					<td width="40%"><?php echo GetMessage("STAT_OPT_ADV_EVENTS_DEFAULT")?></td>
					<td nowrap width="60%"><?php 
						$arr = array(
							"reference" => array(
			...
			?>
			<tr>
				<td width="40%" <?php if($type[0]=="textarea") echo 'class="adm-detail-valign-top"'?>><label for="<?php echo htmlspecialcharsbx($Option[0])?>"><?php echo $Option[1]?></label></td>
				<td nowrap width="60%">
					<?php if($type[0]=="checkbox"):?>
						<input type="checkbox" name="<?php echo htmlspecialcharsbx($Option[0])?>" id="<?php echo htmlspecialcharsbx($Option[0])?>" value="Y"<?php if($val=="Y")echo" checked";?>>
					<?php elseif($type[0]=="text"):?>
	...
	<tr>
		<td colspan="2">
			<?php echo GetMessage("STAT_OPT_CITY_HEADER")?><br><br>
			<table class="internal">
				<tr class="heading">
					<td><?php echo GetMessage("STAT_OPT_CITY_SOURCE")?></td>
					<td><?php echo GetMessage("STAT_OPT_CITY_AVAILABLE")?></td>
					<td><?php echo GetMessage("STAT_OPT_CITY_COUNTRY_LOOKUP")?></td>
					<td><?php echo GetMessage("STAT_OPT_CITY_CITY_LOOKUP")?></td>
					<td><?php echo GetMessage("STAT_OPT_CITY_IS_IN_USE")?></td>
				</tr>
				<?php 
				...
				foreach (GetModuleEvents($module_id, "OnCityLookup", true) as $arEvent):
					...
					<tr>
						<td><?php echo $arDescr["DESCRIPTION"]?></td>
						<td style="text-align:center"><?php echo $arDescr["IS_INSTALLED"]? GetMessage("MAIN_YES"): GetMessage("MAIN_NO")?></td>
						<td style="text-align:center"><?php echo $arDescr["IS_INSTALLED"]? ($arDescr["CAN_LOOKUP_COUNTRY"]? GetMessage("MAIN_YES"): GetMessage("MAIN_NO")): "-"?></td>
						<td style="text-align:center"><?php echo $arDescr["IS_INSTALLED"]? ($arDescr["CAN_LOOKUP_CITY"]? GetMessage("MAIN_YES"): GetMessage("MAIN_NO")): "-"?></td>
						<td style="text-align:center"><input type="radio" name="IP_LOOKUP_CLASS" value="<?php echo $arDescr["CLASS"]?>" <?php echo ($arDescr["CLASS"] == $selected? "checked": "")?>></td>
					</tr>
				<?php endforeach?>
			</table>
	...
	foreach($arOPTIONS["TAB3"] as $key => $Option):
		if(!is_array($Option)):
			...
			?>
			<tr>
				<td <?php if($type[0]=="textarea") echo 'class="adm-detail-valign-top"'?> width="40%"><label for="<?php echo htmlspecialcharsbx($Option[0])?>"><?php echo $Option[1]?></label></td>
				<td nowrap width="60%">
					<?php if($type[0]=="checkbox"):?>
						<input type="checkbox" name="<?php echo htmlspecialcharsbx($Option[0])?>" id="<?php echo htmlspecialcharsbx($Option[0])?>" value="Y"<?php if($val=="Y")echo" checked";?>>
					<?php elseif($type[0]=="text"):?>
	...
	foreach($arOPTIONS["TAB2"] as $key => $Option):
		...
		<?php else:
			...
			?>
			<tr>
				<td width="40%"><?php echo $Option[1]?></td>
				<td nowrap width="60%">
					<?php if($type[0]=="text"):
						...
						if(strlen($Option[3]) > 0):?>
							&nbsp;<label for="<?php echo htmlspecialcharsbx($Option[0])?>_clear"><?php echo GetMessage("STAT_OPT_TIME_CLEAR")?>:</label>&nbsp;<input type="checkbox" name="<?php echo htmlspecialcharsbx($Option[0])?>_clear" id="<?php echo htmlspecialcharsbx($Option[0])?>_clear" value="Y">
						<?php endif;?>
						<?php if(strlen($Option[4]) > 0):?>
							&nbsp;&nbsp;(<?php echo GetMessage("STAT_OPT_TIME_RECORDS")?>&nbsp;<?php echo $count?>)
						<?php endif;?>
					<?php endif?>
				</td>
			</tr>
	...
	foreach($arOPTIONS["TAB4"] as $key => $Option):
		if(!is_array($Option)):
			if ($key=="SKIP_STATISTIC_WHAT"):?>
				<tr>
					<td class="adm-detail-valign-top" width="40%"><?php echo GetMessage("STAT_OPT_SKIP_RULES")?>:</td>
					<td width="60%">
			...
			<?php elseif($key == "SKIP_STATISTIC_GROUPS"):
			...
			<?php elseif($key == "SKIP_STATISTIC_IP_RANGES"):?>
				<tr>
	...
	<input <?php if ($STAT_RIGHT<"W") echo "disabled" ?> type="submit" name="Update" value="<?=GetMessage("MAIN_SAVE")?>" title="<?=GetMessage("MAIN_OPT_SAVE_TITLE")?>" class="adm-btn-save">
	<input <?php if ($STAT_RIGHT<"W") echo "disabled" ?> type="submit" name="Apply" value="<?=GetMessage("MAIN_OPT_APPLY")?>" title="<?=GetMessage("MAIN_OPT_APPLY_TITLE")?>">
	<?php if(strlen($_REQUEST["back_url_settings"])>0):?>
		<input <?php if ($STAT_RIGHT<"W") echo "disabled" ?> type="button" name="Cancel" value="<?=GetMessage("MAIN_OPT_CANCEL")?>" title="<?=GetMessage("MAIN_OPT_CANCEL_TITLE")?>" onclick="window.location='<?php echo htmlspecialcharsbx(CUtil::addslashes($_REQUEST["back_url_settings"]))?>'">
		<input type="hidden" name="back_url_settings" value="<?=htmlspecialcharsbx($_REQUEST["back_url_settings"])?>">
	<?php endif?>
	<input <?php if ($STAT_RIGHT<"W") echo "disabled" ?> type="submit" name="RestoreDefaults" title="<?php echo GetMessage("MAIN_HINT_RESTORE_DEFAULTS")?>" OnClick="return confirm('<?php echo GetMessageJS("MAIN_HINT_RESTORE_DEFAULTS_WARNING")?>')" value="<?php echo GetMessage("MAIN_RESTORE_DEFAULTS")?>">
	<?=bitrix_sessid_post();?>
	<?php $tabControl->End();?>
	<?php if(strlen($_REQUEST["back_url_settings"])>0):?>
		<input type="hidden" name="back_url_settings" value="<?=htmlspecialcharsbx($_REQUEST["back_url_settings"])?>">
	...
	<a name="services"></a>
	<h2><?php echo GetMessage("STAT_OPT_SYSTEM_PROC")?></h2>

	<?php $tabControl2->Begin();?>

	<form name="cleanupform" method="POST" action="<?php echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialcharsbx($mid)?>&amp;lang=<?=LANGUAGE_ID?>">
	<?php $tabControl2->BeginNextTab();?>
		<tr>
			<td width="40%" nowrap><?php echo GetMessage("STAT_OPT_CLEANUP_DATE2")?></td>
			<td width="60%"><?php echo CalendarDate("cleanup_date", htmlspecialcharsbx($cleanup_date), "cleanupform", "10")?></td>
		</tr>
		<tr>
			<td align="left" colspan="2"><input type="button" <?php if ($STAT_RIGHT<"W") echo "disabled" ?> name="cleanup" value="<?php echo GetMessage("STAT_OPT_CLEANUP_BUTTON")?>" OnClick="javascript: CleanUpSubmit();"><input type="hidden" name="cleanup" value="Y"><input type="hidden" name="lang" value="<?=LANGUAGE_ID?>"></td>
		</tr>
	<?php $tabControl2->EndTab();?>
	<script language="JavaScript">
	...
	<?php if (strtolower($statDB->type)=="mysql"):?>
		<form name="optimizeform" method="POST" action="<?php echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialcharsbx($mid)?>&amp;lang=<?=LANGUAGE_ID?>">
		<?php $tabControl2->BeginNextTab();?>
		<tr>
			<td align="left" colspan="2">
				<input type="button" <?php if ($STAT_RIGHT<"W") echo "disabled" ?> name="cleanup" value="<?php echo GetMessage("STAT_OPT_OPTIMIZE_BUTTON")?>" OnClick="javascript: OptimizeSubmit();"><input type="hidden" name="optimize" value="Y"><input type="hidden" name="lang" value="<?=LANGUAGE_ID?>">
				<SCRIPT LANGUAGE="JavaScript">
	...
	<?php endif;?>
	<?php if($STAT_RIGHT>="W" && $bCheckForDDL):?>
		<form name="optimizeform" method="POST" action="<?php echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialcharsbx($mid)?>&amp;lang=<?=LANGUAGE_ID?>">
		<?php $tabControl2->BeginNextTab();?>
		<tr>
			<td align="left" colspan="2">
				...
				endforeach;?>
				<br>
				<input type="submit" name="runsql" value="<?php echo GetMessage("STAT_OPT_INDEX_CREATE_BUTTON")?>">
				<input type="hidden" name="runsql" value="Y">
				<input type="hidden" name="tabControl2_active_tab" value="fedit4">
			<br>
			<?php echo BeginNote('width="100%"');?>
			<span class="required"><?php echo GetMessage("STAT_OPT_INDEX_ATTENTION")?></span> - <?php echo GetMessage("STAT_OPT_INDEX_ATTENTION_DETAIL")?>
			<?php echo EndNote();?>
			</td>
		</tr>
		<?php $tabControl2->EndTab();?>
