...
<input type="hidden" name="type" value="<?php echo htmlspecialcharsbx($_REQUEST["type"])?>" />
...
?>
	<tr>
		<td><?php echo GetMessage("EVENT_NAME")?></td>
		<td><?php 
			...
			if($ID>0 && $COPY_ID<=0)
			{
				...
				?><input type="hidden" name="EVENT_NAME" value="<?php  echo $str_EVENT_NAME?>"><a href="type_edit.php?EVENT_NAME=<?php  echo $str_EVENT_NAME?>"><?php echo $type_NAME?></a> [<?php  echo $str_EVENT_NAME?>]<?php 
			}
			else
			{
				$id_1st = false;
	...
	<tr>
		<td width="40%"><?php echo GetMessage('LAST_UPDATE')?></td>
		<td width="60%"><?php echo $str_TIMESTAMP_X?></td>
	</tr>
	<?php  endif; ?>
	<tr>
		<td><label for="active"><?php echo GetMessage('ACTIVE')?></label></td>
		<td><input type="checkbox" name="ACTIVE" id="active" value="Y"<?php if($str_ACTIVE=="Y")echo " checked"?>></td>
	</tr>
	<tr class="adm-detail-required-field">
		<td class="adm-detail-valign-top"><?php echo GetMessage('LID')?></td>
		<td><?=CLang::SelectBoxMulti("LID", $str_LID);?></td>
	</tr>
	<tr>
		<td><?php echo GetMessage("main_mess_edit_lang")?></td>
		<td>
			<select name="LANGUAGE_ID">
				<option value=""><?php echo GetMessage("main_mess_edit_lang_not_set")?></option>
				<?php 
				...
				<?php  while($language = $languages->fetch()): ?>
					<option value="<?=$language["LID"]?>"<?php  if($str_LANGUAGE_ID == $language["LID"]) echo " selected" ?>>
	...
	<tr class="heading">
		<td colspan="2"><?php echo GetMessage("main_mess_edit_fields")?></td>
	</tr>
	<tr class="adm-detail-required-field">
		<td><?php  echo GetMessage('MSG_EMAIL_FROM')?></td>
		<td><input type="text" name="EMAIL_FROM" size="50" maxlength="255" value="<?php echo $str_EMAIL_FROM?>" onfocus="t=this">
		</td>
	</tr>
	<tr class="adm-detail-required-field">
		<td><?php echo GetMessage('MSG_EMAIL_TO')?></td>
		<td><input type="text" name="EMAIL_TO" size="50" maxlength="255" value="<?php echo $str_EMAIL_TO?>" onfocus="t=this"></td>
	</tr>
	...
	if(!$show_ext):
		...
		<tr id="msg_ext0">
			<td></td>
			<td><a href="javascript:void(0)" onclick="return ShowExtH()"><?php echo GetMessage("MSG_EXT")?></a></td>
		</tr>
		<?php 
	endif;
	?>
	...
	<tr id="msg_ext1" <?=$str_show_ext?>>
		<td><?php echo GetMessage("MSG_CC")?></td>
		<td><input type="text" name="CC" size="50" maxlength="255" value="<?php echo $str_CC?>" onfocus="t=this">
		</td>
	</tr>

	<tr id="msg_ext2" <?=$str_show_ext?>>
		<td><?php echo GetMessage("MSG_BCC")?></td>
		<td><input type="text" name="BCC" size="50" value="<?php echo $str_BCC?>" onfocus="t=this">
		</td>
	</tr>

	<tr id="msg_ext3" <?=$str_show_ext?>>
		<td><?php echo GetMessage("MSG_REPLY_TO")?></td>
		<td><input type="text" name="REPLY_TO" size="50" maxlength="255" value="<?php echo $str_REPLY_TO?>" onfocus="t=this">
		</td>
	</tr>

	<tr id="msg_ext4" <?=$str_show_ext?>>
		<td><?php echo GetMessage("MSG_IN_REPLY_TO")?></td>
		<td><input type="text" name="IN_REPLY_TO" size="50" maxlength="255" value="<?php echo $str_IN_REPLY_TO?>" onfocus="t=this">
		</td>
	</tr>

	<tr id="msg_ext5" <?=$str_show_ext?>>
		<td><?php echo GetMessage("MSG_PRIORITY")?></td>
		<td>
		<input type="text" name="PRIORITY" id="MSG_PRIORITY" size="10" maxlength="255" value="<?php echo $str_PRIORITY?>" onfocus="t=this">
		<select onchange="document.getElementById('MSG_PRIORITY').value=this.value">
			<option value=""></option>
			<option value="1 (Highest)"<?php if($str_PRIORITY=='1 (Highest)')echo ' selected'?>><?php echo GetMessage("MSG_PRIORITY_1")?></option>
			<option value="3 (Normal)"<?php if($str_PRIORITY=='3 (Normal)')echo ' selected'?>><?php echo GetMessage("MSG_PRIORITY_3")?></option>
			<option value="5 (Lowest)"<?php if($str_PRIORITY=='5 (Lowest)')echo ' selected'?>><?php echo GetMessage("MSG_PRIORITY_5")?></option>
		</select>
		</td>
	</tr>
	...
	?>
	<?php foreach($arADDITIONAL_FIELD as $additionalField):?>
		<tr id="msg_ext<?=++$msg_ext?>" <?=$str_show_ext?>>
			<td>
				<input type="text" name="ADDITIONAL_FIELD[NAME][]" size="20" maxlength="255" value="<?php echo htmlspecialcharsbx($additionalField['NAME'])?>" onfocus="t=this">:
			</td>
			<td>
				<input type="text" name="ADDITIONAL_FIELD[VALUE][]" size="55" maxlength="255" value="<?php echo htmlspecialcharsbx($additionalField['VALUE'])?>" onfocus="t=this">
			</td>
		</tr>
	<?php endforeach;?>
	...
	<tr>
		<td><?php echo GetMessage("SUBJECT")?></td>
		<td><input type="text" name="SUBJECT" size="50" maxlength="255" value="<?php echo $str_SUBJECT?>" onfocus="t=this"></td>
	</tr>
	<tr class="heading">
		<td colspan="2"><?=GetMessage("MSG_BODY")?></td>
	</tr>
	<tr>
		<td><?php  echo GetMessage('MSG_SITE_TEMPLATE_ID')?></td>
		<td>
	...
	?>
	<?php if(count($arAttachedImagePlaceHolders)>0):?>
	<tr>
	...
	<tr>
		<td align="left" colspan="2"><br><b><?=GetMessage("AVAILABLE_FIELDS")?></b><br><br>
			<?php echo ReplaceVars(nl2br(trim($type_DESCRIPTION)."\r\n".$str_def));?>
			<?=BeginNote()?>
				<?php echo GetMessage("main_message_edit_html_note")?>
			<?=EndNote()?>
		</td>
	</tr>
