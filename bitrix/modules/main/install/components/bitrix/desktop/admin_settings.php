...
if ($_POST["type"] == "desktop")
{
	...
	else
	{
		...
		<form method="POST" name="frm_desktop_settings" id="frm_desktop_settings" action="<?php  echo $APPLICATION->GetCurPageParam(); ?>" enctype="multipart/form-data">
	...
	if($p !== false)
	{
		...
		if($arGadget)
		{
			...
			else
			{
				...
				?>
				<div class="bx-core-dialog-content">
				<form method="POST" name="frm_desktop_settings" id="frm_desktop_settings" action="<?php  echo $APPLICATION->GetCurPageParam("", array("refresh")); ?>" enctype="multipart/form-data">
				<?php  echo bitrix_sessid_post(); ?>
				...
				foreach($arGadgetParams as $param_id => $arGadgetParam)
				{
					...
					<tr class="bx-gd-admin-settings-col">
						...
						elseif($arGadgetParam["TYPE"] == "LIST")
						{
							...
							?><select style="width:100%" name="GP_<?=$input_id?><?=($arGadgetParam["MULTIPLE"]=="Y"?'[]':'')?>"<?=($arGadgetParam["MULTIPLE"]=="Y"?' multiple="multiple"':'')?><?php if($arGadgetParam["REFRESH"] == "Y"):?> onchange="BX.WindowManager.Get().PostParameters('refresh=Y')"<?php endif;?>><?php 
