...
else
{
	...
	if ($arParams["INLINE"] != "Y")
	{
		...
		if ($arParams["USE_THUMBNAIL_LIST"] == "Y")
		{
			...
			{
				?><a href="<?=$arResult["User"]["HREF"]?>"<?=($arParams["SEO_USER"] == "Y" ? ' rel="nofollow"' : '')?>><?=$arResult["User"]["PersonalPhotoImgThumbnail"]["Image"]?></a><?php 
			}
			elseif (
				strlen($arResult["User"]["DETAIL_URL"]) > 0 
				&& $arResult["CurrentUserPerms"]["Operations"]["viewprofile"]
			...
			{
				?><a href="<?=$arResult["User"]["DETAIL_URL"]?>"<?=($arParams["SEO_USER"] == "Y" ? ' rel="nofollow"' : '')?>><?=$arResult["User"]["PersonalPhotoImgThumbnail"]["Image"]?></a><?php 
			}
			else
			{
		...
		if (strlen($arResult["User"]["HREF"]) > 0)
		{
			?><a class="bx-user-info-name" href="<?=$arResult["User"]["HREF"]?>"<?=($arParams["SEO_USER"] == "Y" ? ' rel="nofollow"' : '')?>><?=$arResult["User"]["NAME_FORMATTED"]?></a><?php 
		}
		elseif (
			strlen($arResult["User"]["DETAIL_URL"]) > 0 
			&& $arResult["CurrentUserPerms"]["Operations"]["viewprofile"]
		...
		{
			?><a class="bx-user-info-name" href="<?=$arResult["User"]["DETAIL_URL"]?>"<?=($arParams["SEO_USER"] == "Y" ? ' rel="nofollow"' : '')?>><?=$arResult["User"]["NAME_FORMATTED"]?></a><?php 
		}
		else
		{
			?><div class="bx-user-info-name"><?=$arResult["User"]["NAME_FORMATTED"]?></div><?php 
		}
		?><?=(strlen($arResult["User"]["NAME_DESCRIPTION"]) > 0 ? " (".$arResult["User"]["NAME_DESCRIPTION"].")": "")?><?php 
		if ($arResult["bSocialNetwork"])
		{
			...
			{
				?><div id="<?=$arResult["User"]["HTML_ID"]?>"<?=$online_class_attrib?>><a href="<?=$link?>"><img src="/bitrix/images/1.gif" width="11" height="11" border="0"></a></div><?php 
			}
			?></div><?php 
		}
		?></td>
	...
	else
	{
		...
		{
			?><a href="<?=$arResult["User"]["DETAIL_URL"]?>"<?=($arParams["SEO_USER"] == "Y" ? ' rel="nofollow"' : '')?> id="anchor_<?=$anchor_id?>" bx-tooltip-user-id="<?=$arResult["User"]["ID"]?>"><?=$arResult["User"]["NAME_FORMATTED"]?></a><?php 
		}
		elseif (strlen($arResult["User"]["DETAIL_URL"]) > 0 && !$arResult["bSocialNetwork"])
		{
			?><a href="<?=$arResult["User"]["DETAIL_URL"]?>"<?=($arParams["SEO_USER"] == "Y" ? ' rel="nofollow"' : '')?> id="anchor_<?=$anchor_id?>"><?=$arResult["User"]["NAME_FORMATTED"]?></a><?php 
		}
		else
		{
