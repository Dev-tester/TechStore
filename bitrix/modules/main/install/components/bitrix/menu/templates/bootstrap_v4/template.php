...
<div class="bx-top-nav bx-<?=$arParams["MENU_THEME"]?>" id="<?=$menuBlockId?>">
	<nav class="bx-top-nav-container" id="cont_<?=$menuBlockId?>">
		...
		<?php foreach($arResult["MENU_STRUCTURE"] as $itemID => $arColumns):?>     <!-- first level-->
			...
			<li
				class="bx-nav-1-lvl bx-nav-list-<?=($existPictureDescColomn) ? count($arColumns)+1 : count($arColumns)?>-col <?php if($arResult["ALL_ITEMS"][$itemID]["SELECTED"]):?>bx-active<?php endif?><?php if (is_array($arColumns) && count($arColumns) > 0):?> bx-nav-parent<?php endif?>"
			...
			>
				<a
					...
					<?php if (is_array($arColumns) && count($arColumns) > 0 && $existPictureDescColomn):?>
				...
				>
					<span class="bx-nav-1-lvl-link-text">
						...
						<?php if (is_array($arColumns) && count($arColumns) > 0):?> <i class="bx-nav-angle-bottom"></i><?php endif?>
					</span>
				</a>
			<?php if (is_array($arColumns) && count($arColumns) > 0):?>
