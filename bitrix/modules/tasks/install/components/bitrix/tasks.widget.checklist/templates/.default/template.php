...
<?php if(!$helper->checkHasFatals()):?>
	...
	<div id="<?=$helper->getScopeId()?>" class="tasks<?php if(!$arParams['CAN_REORDER']):?> nodrag<?php endif?> tasks-wg-checklist">

		<?php //$helper->displayWarnings();?>

		<?php $inputPrefix = htmlspecialcharsbx($arParams["INPUT_PREFIX"]);?>
		<?php $tData = $arResult['TEMPLATE_DATA'];?>
		...
		<?php if($arParams['CAN_ADD']):?>
			<div class="task-checklist-field tasks-checklist-dropzone tasks-checklist-form"><?php 
				?><span class="js-id-checklist-is-add-item-form block-on task-checklist-field-inner-add invisible"><?php 
					?><span class="task-checklist-form-vpadding"><?php 
				...
				?><div class="task-checklist-actions"><?php 
					?><span class="block-off task-dashed-link"><span class="js-id-checklist-is-open-form task-dashed-link-inner"><?=Loc::getMessage('TASKS_TTDP_CHECKLIST_ADD')?></span></span><?php 
					?><span class="js-id-checklist-add-separator task-dashed-link"><span class="js-id-checklist-is-add-separator task-dashed-link-inner"><?=Loc::getMessage('TASKS_TTDP_CHECKLIST_SEPARATOR')?></span></span><?php 
				?></div><?php 
			?></div>
		<?php endif?>
