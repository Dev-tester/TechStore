			...
			<?php elseif($type[0]=="selectbox"):
				?><select name="<?php echo htmlspecialcharsbx($arOption[0])?>"><?php 
					foreach($type[1] as $key => $value):
						?><option value="<?php echo htmlspecialcharsbx($key)?>"<?php if($key==$val) echo ' selected="selected"'?>><?php echo htmlspecialcharsEx($value)?></option><?php 
					endforeach;
				?></select><?php 
			endif?>
		</td>
...
<?php $tabControl->Buttons();?>
	<input <?php if(!$RIGHT_W) echo "disabled" ?> type="submit" name="Update" value="<?=GetMessage("MAIN_SAVE")?>" title="<?=GetMessage("MAIN_OPT_SAVE_TITLE")?>" class="adm-btn-save">
	<input <?php if(!$RIGHT_W) echo "disabled" ?> type="submit" name="Apply" value="<?=GetMessage("MAIN_OPT_APPLY")?>" title="<?=GetMessage("MAIN_OPT_APPLY_TITLE")?>">
	...
	<?php endif?>
	<input <?php if(!$RIGHT_W) echo "disabled" ?> type="submit" name="RestoreDefaults" title="<?php echo GetMessage("MAIN_HINT_RESTORE_DEFAULTS")?>" onclick="return confirm('<?php echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>')" value="<?php echo GetMessage("MAIN_RESTORE_DEFAULTS")?>">
	<?=bitrix_sessid_post();?>
<?php $tabControl->End();?>
</form>
<?php endif;?>
